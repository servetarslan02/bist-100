...
class PositionSizer:
    ...
    def calculate_position_sizes(
        ...
        print(f"  leverage={leverage:.4f} (target_vol={self.target_volatility}, current_vol={current_volatility:.4f})")
        ...
        if calibrator:
            ...
            print(f"  historical_avg_win={hist_avg_win:.4f}, historical_avg_loss={hist_avg_loss:.4f}")
        ...
        for opp in opportunities:
            ...
            print(f"    avg_win={ticker_avg_win:.4f}, avg_loss={ticker_avg_loss:.4f}")
            ...
            if has_history:
                ...
                print(f"    kelly={kelly:.4f} (fractional={self.kelly_fraction})")

                if kelly <= 0:
                    print(f"    -> SKIP: kelly<=0 (negative expectation, NO TRADE is correct)")
                    continue
            ...
            print(f"    vol_adjustment={vol_adj:.4f}")
        ...
        if total_weight > self.max_total_exposure:
            ...
            print(f"  NORMALIZE: scale={scale:.4f}")
    ...
    def _fractional_kelly(self, win_prob: float, avg_win: float, avg_loss: float) -> float:
        ...
        """
        print(f"      [KELLY] p={win_prob:.4f}, avg_win={avg_win:.4f}, avg_loss={avg_loss:.4f}")

        if avg_loss <= 0:
            print(f"      -> avg_loss<=0, kelly=0")
            return 0.0
        if win_prob <= 0 or win_prob >= 1:
            print(f"      -> win_prob out of range, kelly=0")
            return 0.0

        q = 1 - win_prob
        b = avg_win / avg_loss  # Odds

        print(f"      [KELLY] q={q:.4f}, b={b:.4f}")

        raw_kelly = (win_prob * b - q) / b
        print(f"      [KELLY] raw_kelly={raw_kelly:.4f}")

        # raw_kelly negatifse expectation negatif -> NO TRADE (dogru davranis)
        if raw_kelly <= 0:
            print(f"      -> raw_kelly<=0 (negative expectation, NO TRADE)")
            return 0.0

        kelly = max(0, min(1, raw_kelly))
        fractional = kelly * self.kelly_fraction
        print(f"      [KELLY] clamped={kelly:.4f}, fractional={fractional:.4f}")
