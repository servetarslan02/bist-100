...
class TestSyntheticMicrostructure(unittest.TestCase):
    ...
    def test_corwin_schultz_spread_and_bist_floor(self):
        ...
        spread_pct = self.estimator.estimate_corwin_schultz_spread(
            high_prev=102.0, low_prev=98.0,
            high_curr=103.0, low_curr=99.0,
            price=price,
        )
        ...
        spread_flat = self.estimator.estimate_corwin_schultz_spread(
            high_prev=100.0, low_prev=100.0,
            high_curr=100.0, low_curr=100.0,
            price=price,
        )
    ...
    def test_synthetic_order_book_generation_deterministic(self):
        ...
        book1 = self.builder.build_synthetic_book(
            ticker="THYAO", mid_price=250.0, adv=1_000_000,
            volatility=0.25, spread_pct=0.20,
            scenario=LiquidityScenario.NORMAL, num_levels=10,
        )
    ...
    def test_walk_the_book_and_participation_cap(self):
        ...
        book = self.builder.build_synthetic_book(
            ticker="GARAN", mid_price=100.0, adv=adv,
            volatility=0.20, spread_pct=0.20,
            scenario=LiquidityScenario.NORMAL, num_levels=10,
        )
        ...
        res = self.builder.execute_market_order_walk(
            book=book, side="BUY", requested_quantity=60_000,
            adv=adv, scenario=LiquidityScenario.NORMAL,
        )
    ...
    def test_multi_scenario_spread_and_depth_scaling(self):
        ...
        pess_book = self.builder.build_synthetic_book(
            ticker="KCHOL", mid_price=150.0, adv=adv, volatility=0.25,
            spread_pct=1.0, scenario=LiquidityScenario.PESSIMISTIC,
        )
        norm_book = self.builder.build_synthetic_book(
            ticker="KCHOL", mid_price=150.0, adv=adv, volatility=0.25,
            spread_pct=1.0, scenario=LiquidityScenario.NORMAL,
        )
        opt_book = self.builder.build_synthetic_book(
            ticker="KCHOL", mid_price=150.0, adv=adv, volatility=0.25,
            spread_pct=1.0, scenario=LiquidityScenario.OPTIMISTIC,
        )
    ...
    def test_kap_restriction_registry_timestamp_and_gating(self):
        ...
        ok_data, reason_data = self.kap_reg.validate_trading_eligibility("SASA", "2024-01-02", "BUY", data_quality_ok=False)
    ...
    def test_scenario_manager_success_gate(self):
        ...
        pessimistic = ScenarioResult(
            ...
            total_return_pct=5.0, cagr_pct=15.0, sharpe_ratio=0.8,
            max_drawdown_pct=12.0, win_rate=0.55, total_commission=100.0,
            total_slippage_cost=500.0, num_trades=20,
        )
        normal = ScenarioResult(
            scenario=LiquidityScenario.NORMAL,
            total_return_pct=12.0, cagr_pct=30.0, sharpe_ratio=1.4,
            max_drawdown_pct=6.0, win_rate=0.60, total_commission=100.0,
            total_slippage_cost=300.0, num_trades=20,
        )
        optimistic = ScenarioResult(
            scenario=LiquidityScenario.OPTIMISTIC,
            total_return_pct=18.0, cagr_pct=45.0, sharpe_ratio=1.9,
            max_drawdown_pct=4.0, win_rate=0.65, total_commission=100.0,
            total_slippage_cost=150.0, num_trades=20,
        )
        ...
        fake_pess = ScenarioResult(
            scenario=LiquidityScenario.PESSIMISTIC, total_return_pct=-8.0,
            cagr_pct=-15.0, sharpe_ratio=-0.5, max_drawdown_pct=28.0,
            win_rate=0.35, total_commission=100.0, total_slippage_cost=800.0, num_trades=20,
        )
        fake_norm = ScenarioResult(
            scenario=LiquidityScenario.NORMAL, total_return_pct=-2.0,
            cagr_pct=-5.0, sharpe_ratio=-0.1, max_drawdown_pct=18.0,
            win_rate=0.45, total_commission=100.0, total_slippage_cost=400.0, num_trades=20,
        )
