...
def run_phase_23() -> Any:
    ...
    for d, grp in df_all.groupby("date"):
        ...
        t3 = grp.nlargest(3, "signal")["ex_5d"].mean()
        b3 = grp.nsmallest(3, "signal")["ex_5d"].mean()
        t5 = grp.nlargest(5, "signal")["ex_5d"].mean()
        b5 = grp.nsmallest(5, "signal")["ex_5d"].mean()
        t10 = grp.nlargest(10, "signal")["ex_5d"].mean()
        b10 = grp.nsmallest(10, "signal")["ex_5d"].mean()
        ...
        q_rets = (
            {f"Q{i + 1}": grp.loc[q == i, "ex_5d"].mean() for i in range(5)}
            if q is not None
            else {f"Q{i + 1}": 0 for i in range(5)}
        )
        ...
        daily_res.append(
            {
                "date": d,
                "regime": regimes[d],
                "is_high_vol": grp["is_high_vol"].iloc[0],
                "ic_1d": ic_1,
                "ic_5d": ic_5,
                "ic_10d": ic_10,
                "ic_20d": ic_20,
    ...
    for i in range(5):
        logger.info(f"Q{i + 1}: %{res_df[f'Q{i + 1}'].mean():6.3f}")
