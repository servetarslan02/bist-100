...
def test_scenario_engine() -> Any:
    ...
    passed += 1
    logger.info(f"  ✓ Basic scenario (impact: {result.portfolio_impact_pct:.2f}%)")
    ...
    passed += 1
    logger.info(f"  ✓ {len(PREDEFINED_SCENARIOS)} predefined scenarios ({zero_count} zero impact)")
    ...
    passed += 1
    logger.info(f"  ✓ Sector impacts: {result.sector_impacts}")
    ...
    passed += 1
    logger.info(f"  ✓ Stress test ({len(stress_results)} scenarios, worst: {worst.scenario_name})")
    ...
    passed += 1
    logger.info(f"  ✓ Breaking point: {bp.description}")
    ...
    passed += 1
    logger.info("  ✓ Empty positions handled")
