...
def dim1_syntax_and_encoding(rel: str, raw_bytes: bytes) -> list[Finding]:
    ...
    if b"\x00" in raw_bytes:
        finds.append(Finding(1, "NULL_BYTES", "CRITICAL", rel, 1, "Dosya null byte içeriyor — SyntaxError'a yol açar"))
...
class DeepInspector(ast.NodeVisitor):
    ...
    def _check_docstring(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> Any:
        ...
        if not has_doc:
            self._add(
                16, "MISSING_DOCSTRING", "LOW", node.lineno, f"Public fonksiyon '{node.name}' için docstring eksik"
            )
        ...
        for handler in node.handlers:
            ...
            # except Exception: pass veya except SomethingError: pass
                ...
                self._add(
                    ...
                    "HIGH",
                    handler.lineno,
                    "Spesifik istisna yakalanıp sessizce yutulmuş — fail-closed ihlali",
                )
    ...
    def _check_import(self, mod: str, lineno: int, names: list[str] | None = None) -> Any:
        ...
        if self._is_prod and (mod == "pandas" or mod.startswith("pandas.")):
            self._add(
                ...
                "HIGH",
                lineno,
                "Üretim servisinde 'pandas' import edilmiş — proje standardı Polars zorunludur!",
            )
        ...
        if self._is_prod and ("mock" in mod or mod.startswith("unittest.mock")):
            self._add(
                ...
                "CRITICAL",
                lineno,
                "Üretim kodunda mock kütüphanesi tespit edildi — test kodu production'a sızmış!",
            )
    ...
    def visit_Assert(self, node: ast.Assert) -> Any:
        ...
        if isinstance(node.test, ast.Constant) and node.test.value is True:
            self._add(
                7, "FAKE_ASSERT_TRUE", "HIGH", node.lineno, "Sahte assertion: 'assert True' — hiçbir şey test etmiyor!"
            )
...
def dim6_text_scan(rel: str, content: str, lines: list[str]) -> list[Finding]:
    ...
    _insecure_re = re.compile(
        r'["\'](' + "|".join(re.escape(v) for v in INSECURE_DEFAULTS if v) + r')["\']', re.IGNORECASE
    )
    # Görev işaretleyicileri regex
        ...
        if m:
            finds.append(
                Finding(
                    ...
                    rel,
                    idx,
                    f"Tamamlanmamış görev: '{m.group(1)}' işareti bırakılmış",
                    stripped,
                )
            )
                ...
                if not any(kw in line for kw in ("os.getenv", "settings.", "environ", "Field(", "env_var")):
                    finds.append(
                        Finding(
                            ...
                            rel,
                            idx,
                            f"Muhtemel hardcoded kimlik bilgisi: '{val[:8]}...'",
                            stripped,
                        )
                    )
        ...
        if mi and not rel.endswith((".example", "config.py")):
            finds.append(
                Finding(
                    ...
                    rel,
                    idx,
                    f"Güvensiz varsayılan değer kullanılmış: '{mi.group(1)}'",
                    stripped,
                )
            )
        ...
        if is_prod and _print_re.match(line):
            finds.append(
                Finding(
                    ...
                    rel,
                    idx,
                    "Üretim kodunda print() kullanımı — structlog ile değiştirilmeli",
                    stripped,
                )
            )
...
def dim10_architecture_check(all_imports: dict[str, set[str]]) -> list[Finding]:
                    ...
                    if key in REQUIRED_ENV_VARS and not val:
                        finds.append(
                            Finding(
                                ...
                                ".env",
                                1,
                                f"Zorunlu ortam değişkeni '{key}' boş bırakılmış",
                            )
                        )
                    ...
                    if val in INSECURE_DEFAULTS and val:
                        finds.append(
                            Finding(
                                ...
                                ".env",
                                1,
                                f"'{key}' insecure varsayılan değer içeriyor: '{val}'",
                            )
                        )
    else:
        finds.append(Finding(12, "MISSING_DOTENV", "CRITICAL", ".env", 1, ".env dosyası yok! Servisler başlatılamaz."))

    # .env.example'daki tüm değişkenler .env'de var mı?
    if env_example.exists() and env_path.exists():
        example_vars: set[str] = set()
        with open(env_example, encoding="utf-8", errors="replace") as f:
            for line in f:
                line = line.strip()
                if line and not line.startswith("#") and "=" in line:
        ...
        if missing:
            for var in sorted(missing)[:20]:
                finds.append(
                    Finding(
                        12,
                        "ENV_VAR_MISSING_FROM_DOTENV",
                        "MEDIUM",
                        ".env",
                        1,
                        f"'{var}' .env.example'da tanımlı ama .env'de eksik",
                    )
                )
    ...
    for module_path, expected_test in CRITICAL_MODULES.items():
        ...
        if test_name not in existing_tests:
            finds.append(
                Finding(
                    15,
                    "MISSING_TEST_FOR_CRITICAL_MODULE",
                    "HIGH",
                    module_path,
                    1,
...
def run_ultimate_audit() -> Any:
    ...
    logger.info("=" * 70)
    logger.info("  ALPHA BIST — NİHAİ KAPSAMLI ÇAPRAZ DENETİM MOTORU v2.0")
    logger.info("  16 Boyut | 0 Token | Tamamen Yerel AST + Semantik Analiz")
    logger.info("=" * 70)
    ...
    logger.info(f"  Taranan Python dosyasi: {total_files}")
    logger.info("  Tarama boyutu: 16 boyut")
    logger.info("\n  Tarama basliyor...\n")
    ...
    for p in py_files:
        ...
        try:
            ...
            all_findings.append(Finding(1, "FILE_READ_ERROR", "CRITICAL", rel, 1, f"Dosya okunamadi: {e}"))
            continue
        ...
        try:
            ...
            all_findings.append(Finding(1, "PARSE_ERROR", "HIGH", rel, 1, f"AST parse hatasi: {e}"))
            continue
    ...
    logger.info(f"  Tarama tamamlandi: {elapsed:.2f} saniye")
    logger.info(f"  Toplam dosya: {total_files:,}  |  Toplam satir: {total_lines:,}")
    logger.info(f"  KRITIK: {crit}  YUKSEK: {high}  ORTA: {med}  DUSUK: {low}")
    logger.info(f"  Sistem Saglik Puani: {health}/100")
    logger.info()
    ...
    with open(md_path, "w", encoding="utf-8") as mf:
        ...
        w("> **Motor:** Ultimate Audit Engine v2.0 (16 Boyut, 0 Token)")
        ...
        for dim_id, dim_name in DIM_NAMES.items():
            ...
            icon = (
                "X KRITIK"
                if any(f.dim == dim_id and f.severity == "CRITICAL" for f in all_findings)
                else (
                    "! YUKSEK"
                    if any(f.dim == dim_id and f.severity == "HIGH" for f in all_findings)
        ...
        w("*JSON verisi: `audit/ultimate_audit_findings.json`*")

    logger.info(f"  Markdown Raporu: {md_path}")
    logger.info(f"  JSON Verisi:     {json_path}")
    logger.info()
    logger.info("=" * 70)

    # Terminale kisa ozet
    logger.info("\n  KRITIK SORUNLAR (ilk 15):")
    for f in sorted([x for x in all_findings if x.severity == "CRITICAL"], key=lambda x: (x.dim, x.file))[:15]:
        logger.info(f"  [B{f.dim:02d}] {f.file}:{f.line} -> {f.category}: {f.msg[:70]}")
