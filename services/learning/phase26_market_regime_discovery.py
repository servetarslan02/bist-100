...
def evaluate_timing(xu100_df, signal_col, tc=0.001):
    df = xu100_df.copy()
    ...
    return {
        "bh_ret": bh_ret, "bh_sharpe": bh_sharpe, "bh_max_dd": bh_max_dd,
        "ann_ret": ann_ret, "sharpe": sharpe, "max_dd": max_dd,
        "exposure": exposure, "down_cap": down_cap,
        "ci_L": ci_L, "ci_U": ci_U, "pval": pval,
        ...
        for tk, sdf in stock_data.items():
            if d in sdf.index:
                ...
                if len(sdf.loc[:d]) >= 20:
                    ...
                    if c > sma: above += 1
                    total += 1
        if total > 0: breadth.loc[d] = above / total
