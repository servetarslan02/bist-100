...
def calculate_capture_ratios(port_rets, bench_rets):
    ...
    down_capture = (down_market["port"].mean() / down_market["bench"].mean()) * 100 if down_market["bench"].mean() < 0 else 0
    return up_capture, down_capture

def run_mechanism_candidate(eval_dates, features_by_ticker, xu100_close, trainer, candidate_name):
    ...
    daily_rets = []
    exposure_history = []
    regime_pnl = {"BULL_TREND": 0, "BEAR_MARKET": 0, "SIDEWAYS_RANGE": 0, "HIGH_VOLATILITY": 0, "LOW_VOLATILITY": 0, "V_DIP_RECOVERY": 0}
    ...
    for step_i, current_date in enumerate(eval_dates):
        ...
        for tk, pos in list(positions.items()):
            ...
            if candidate_name == "B_Signal_Decay":
                ...
                if smoothed_scores[tk] < entry_thresh: should_exit = True
            else:
                # V3 Default Signal Exit
                if pos["days_held"] >= FROZEN_PARAMS["min_hold_days"] and smoothed_scores[tk] < FROZEN_PARAMS["signal_reversal_thresh"]: should_exit = True
            ...
            if candidate_name == "A_Conviction_Alloc":
                ...
                for c in selected:
                    ...
                    if shares > 0:
                        ...
                        positions[c["ticker"]] = {"shares": shares, "entry_price": c["close"], "days_held": 0, "highest_price": c["close"], "lowest_price": c["close"], "atr_pct": c["atr_pct"], "regime": regime_tag}
            else:
    ...
    return {
        "Ret": ret_total, "CAGR": cagr, "MaxDD": max_dd, "Sharpe": sharpe, "Sortino": sortino, "Calmar": calmar,
        "PF": pf, "WR": win_rate, "UpCap": up_cap, "DnCap": down_cap, "Trades": trades, "Cost": total_costs,
        "AvgExp": avg_exp, "WorstTrade": worst_trade, "WorstMonth": worst_month, "RegimePnL": regime_pnl
    }
