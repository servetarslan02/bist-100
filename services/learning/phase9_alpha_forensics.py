...
def analyze_forensics(df, feature_cols) -> Any:
    ...
    for f in feature_cols + ["score"]:
        ...
        ic_results.append({"Feature": f, "Overall": overall_ic, "Bull": bull_ic, "Bear": bear_ic, "Sideways": side_ic})

    df_ic = pd.DataFrame(ic_results)
    for index, row in df_ic.iterrows():
        alert = (
            " ⚠️ TERSINE DONUS"
            if ((row["Bull"] < -0.02 and row["Overall"] > 0) or (row["Overall"] < 0 and row["Bull"] > 0.02))
            else ""
        )
        if row["Feature"] == "score":
            alert = " 🎯 MODEL SCORE"
        logger.info(
            f"{row['Feature']:<16} | Overall: {row['Overall']:>6.3f} | Bull: {row['Bull']:>6.3f} | Bear: {row['Bear']:>6.3f} | Side: {row['Sideways']:>6.3f}{alert}"
        )
    ...
    if not bull_df.empty:
        ...
        monthly_ic = bull_df.groupby("month").apply(
            lambda x: spearmanr(x["score"], x["fwd_5d"])[0] if len(x) > 20 else np.nan
        )
...
if __name__ == "__main__":
    ...
    logger.info(
        "Interpretation   : Score 0.30 means the model literally predicts +30% return in 5 days. It is NOT a confidence probability."
    )
