...
def test_cross_sectional():
    ...
    assert "sector_momentum_TECH" in sector_mom
    assert "sector_momentum_BANK" in sector_mom
    passed += 1
    print(f"  ✓ Sector momentum: TECH={sector_mom.get('sector_momentum_TECH', 0):.1f}, BANK={sector_mom.get('sector_momentum_BANK', 0):.1f}")
...
def test_mask_integration():
    ...
    df = pl.DataFrame({
        ...
        "high": m_high.tolist(),
        "low": m_low.tolist(),
        "close": m_close.tolist(),
        "volume": m_volume.tolist(),
    })
