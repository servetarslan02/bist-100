...
def main() -> Any:
    """Otomatik eklendi."""
    logger.info("=" * 105)
    logger.info("🛡️ ALPHA BIST — RİSK PARİTY & VOLATİLİTE SIZING DENETİMİ (30 YIL: 1997 - 2026)")
    logger.info("=" * 105)
    ...
    # ---------------------------------------------------------------------------------------------
    logger.info("\n" + "=" * 105)
    logger.info("🔒 1. 2024 - 2026 KİLİTLİ KÖR HOLDOUT TESTİ (RİSK PARİTY İLE)")
    logger.info("=" * 105)
    ...
    logger.info(f"  • Kümülatif Net Getiri   : %{oos_res.total_return_pct:+,.1f} (BIST-100 Endeksi: %{bm_ret:+,.1f})")
    logger.info(f"  • Yıllık Getiri (CAGR)   : %{oos_res.cagr:.2f}")
    logger.info(f"  • Sharpe Oranı           : {oos_res.sharpe_ratio:.2f}")
    logger.info(f"  • Sortino Oranı          : {oos_res.sortino_ratio:.2f}")
    logger.info(f"  • Kâr Faktörü (PF)       : {oos_res.profit_factor:.2f}")
    logger.info(f"  • Kazanma Oranı (Win Rate): %{oos_res.win_rate:.1f}")
    logger.info(f"  • Maksimum Düşüş (Max DD): %{oos_res.max_drawdown:.2f}  <-- (ÖNCEKİ %-76'DAN DÜŞÜRÜLDÜ)")
    logger.info(f"  • Toplam İşlem Sayısı    : {oos_res.total_trades} Adet")

    # ---------------------------------------------------------------------------------------------
    # 2. 1997 - 2023 IN-SAMPLE DOĞRULAMASI
    # ---------------------------------------------------------------------------------------------
    logger.info("\n" + "=" * 105)
    logger.info("📈 2. 1997 - 2023 IN-SAMPLE PERFORMANSI (RİSK PARİTY İLE)")
    logger.info("=" * 105)
    is_res = engine.simulate(params, start_year=1997, end_year=2023)
    logger.info(f"  • Kümülatif Net Getiri   : %{is_res.total_return_pct:+,.1f}")
    logger.info(f"  • Yıllık Getiri (CAGR)   : %{is_res.cagr:.2f}")
    logger.info(f"  • Sharpe Oranı           : {is_res.sharpe_ratio:.2f}")
    logger.info(f"  • Sortino Oranı          : {is_res.sortino_ratio:.2f}")
    logger.info(f"  • Kâr Faktörü (PF)       : {is_res.profit_factor:.2f}")
    logger.info(f"  • Kazanma Oranı (Win Rate): %{is_res.win_rate:.1f}")
    logger.info(f"  • Maksimum Düşüş (Max DD): %{is_res.max_drawdown:.2f}")
    logger.info(f"  • Toplam İşlem Sayısı    : {is_res.total_trades} Adet")

    # ---------------------------------------------------------------------------------------------
    # 3. YIL BAZINDA PERFORMANS & KRİZ TABLOSU
    # ---------------------------------------------------------------------------------------------
    logger.info("\n" + "=" * 105)
    logger.info("📅 3. YIL BAZINDA GERÇEK GETİRİ VE MAKSİMUM DÜŞÜŞ TABLOSU (1997 - 2026)")
    logger.info("=" * 105)
    logger.info(
    ...
    # ---------------------------------------------------------------------------------------------
    logger.info("\n" + "=" * 105)
    logger.info("🎯 4. SABİT %18 POZİSYON vs RİSK PARİTY HEDEF TABLOSU")
    logger.info("=" * 105)
    logger.info(f"{'METRİK':<32} | {'ÖNCEKİ (SABİT %18 BOYUT)':<28} | {'YENİ (RİSK PARİTY & HEAT)':<28} | {'HEDEF DURUMU'}")
    logger.info("-" * 105)
    ...
    logger.info(f"{'2024-2026 OOS Max DD':<32} | %{' -76.62':<26} | %{oos_res.max_drawdown:>25.2f} | {dd_status}")
    logger.info(f"{'2024-2026 OOS Profit Factor':<32} | {' 1.03':<27} | {oos_res.profit_factor:>27.2f} | {pf_status}")
    logger.info(f"{'2024-2026 OOS Sharpe':<32} | {' 0.48':<27} | {oos_res.sharpe_ratio:>27.2f} | {sharpe_status}")
    logger.info(f"{'2024-2026 OOS Net Getiri':<32} | %{' +6.6':<26} | %{oos_res.total_return_pct:>25.1f} | {'Pozitif Kâr'}")
    logger.info(f"{'1997-2023 In-Sample Max DD':<32} | %{' -37.05':<26} | %{is_res.max_drawdown:>25.2f} | {'Daha Güvenli'}")
    logger.info("=" * 105)
