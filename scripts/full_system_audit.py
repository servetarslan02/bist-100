...
class AuditReport:
    ...
    def print_report(self):
        ...
        total_issues = sum(len(v) for v in [
            self.critical_bugs, self.logic_bugs, self.financial_math_bugs,
            self.data_bugs, self.leakage_risks, self.security_bugs,
            self.performance_issues, self.missing_features
        ])
        print(f"  Toplam Bulgu: {total_issues}")
        print(f"  P0 (Kritik): {sum(1 for v in [self.critical_bugs, self.security_bugs] for i in v if i.severity == 'P0')}")
        print(f"  P1 (Yüksek): {sum(1 for v in [self.logic_bugs, self.financial_math_bugs, self.leakage_risks] for i in v if i.severity == 'P1')}")
        print(f"  P2 (Orta): {sum(1 for v in [self.data_bugs, self.performance_issues] for i in v if i.severity == 'P2')}")
    ...
    try:
        ...
        if not market_data:
            issues.append(AuditIssue(
                module="Data", severity="P0", category="CRITICAL BUG",
        ...
        for ticker, df in market_data.items():
            if df is None or df.empty:
                issues.append(AuditIssue(
                    module="Data", severity="P1", category="DATA BUG",
        ...
        mask_result = data_quality.check_tradability(
            ticker="TEST",
            open_price=0, high=-1, low=-2, close=-1, volume=0, prev_close=100
        )
        if mask_result.is_tradable:
            issues.append(AuditIssue(
                module="Data Quality", severity="P0", category="CRITICAL BUG",
...
def audit_data_quality(report: AuditReport):
    ...
    try:
        ...
        mask = dq.check_tradability(
            ticker="ANOMALY",
            open_price=100, high=200, low=50, close=150,
            volume=1000000, prev_close=100
        )
        if mask.is_tradable:
            issues.append(AuditIssue(
                module="Data Quality", severity="P1", category="LOGIC BUG",
...
def audit_tradability_mask(report: AuditReport):
    ...
    try:
        ...
        if mask_result.mask[0] != 0:
            issues.append(AuditIssue(
                module="Tradability Mask", severity="P0", category="CRITICAL BUG",
        ...
        if mask_result.mask[1] != 0:
            issues.append(AuditIssue(
                module="Tradability Mask", severity="P1", category="LOGIC BUG",
...
def audit_features(report: AuditReport):
    ...
    try:
        ...
        if not features:
            issues.append(AuditIssue(
                module="Features", severity="P0", category="CRITICAL BUG",
        ...
        for feat_name in expected_features:
            if feat_name in features:
                ...
                if isinstance(val, np.ndarray):
                    ...
                    if nan_ratio > 0.5:
                        issues.append(AuditIssue(
                            module="Features", severity="P1", category="DATA BUG",
                ...
                issues.append(AuditIssue(
                    module="Features", severity="P2", category="MISSING FEATURE",
            ...
            if isinstance(rsi, np.ndarray):
                ...
                if len(valid_rsi) > 0:
                    if np.any(valid_rsi < 0) or np.any(valid_rsi > 100):
                        issues.append(AuditIssue(
                            module="Features", severity="P0", category="FINANCIAL MATH BUG",
        ...
        if 'macd' in features and 'macd_signal' in features:
            ...
            if isinstance(macd, np.ndarray) and isinstance(signal, np.ndarray):
                ...
                if len(macd) != len(signal):
                    issues.append(AuditIssue(
                        module="Features", severity="P1", category="LOGIC BUG",
    ...
    try:
        ...
        if cs:
            ...
            rank_feats = cross_sectional_engine.compute_rank_features(
                "A", universe_features["A"], universe_features
            )
            if rank_feats:
            ...
            issues.append(AuditIssue(
                module="Cross Sectional", severity="P2", category="MISSING FEATURE",
                ...
                if key in cs_rev:
                    if isinstance(cs[key], (int, float)) and isinstance(cs_rev[key], (int, float)):
                        if abs(cs[key] - cs_rev[key]) > 0.001:
                            issues.append(AuditIssue(
                                module="Cross Sectional", severity="P0", category="CRITICAL BUG",
        ...
        issues.append(AuditIssue(
            module="Cross Sectional", severity="P1", category="CRITICAL BUG",
...
def audit_regime(report: AuditReport):
    ...
    try:
        ...
        if regime.regime not in ["BULL", "UNKNOWN"]:
            issues.append(AuditIssue(
                module="Regime", severity="P1", category="LOGIC BUG",
        ...
        if regime.transition_probability:
            ...
            if abs(total_prob - 1.0) > 0.1:
                issues.append(AuditIssue(
                    module="Regime", severity="P2", category="LOGIC BUG",
        ...
        issues.append(AuditIssue(
            module="Regime", severity="P1", category="CRITICAL BUG",
    ...
    try:
        ...
        for score in test_scores:
            ...
            if prob is not None:
                ...
                if prob < 0 or prob > 1:
                    issues.append(AuditIssue(
                        module="Calibration", severity="P0", category="FINANCIAL MATH BUG",
        ...
        issues.append(AuditIssue(
            module="Calibration", severity="P1", category="CRITICAL BUG",
...
def audit_position_sizing(report: AuditReport):
    ...
    try:
        ...
        if positions:
            ...
            details_lines.append(f"  win_prob={pos.win_probability:.4f}, avg_win={pos.avg_win:.4f}, avg_loss={pos.avg_loss:.4f}")

            # Negatif weight kontrolü
            if pos.weight < 0:
                issues.append(AuditIssue(
                    module="Position Sizing", severity="P0", category="FINANCIAL MATH BUG",
            ...
            if pos.weight > ps.max_position_pct:
                issues.append(AuditIssue(
                    module="Position Sizing", severity="P1", category="FINANCIAL MATH BUG",
        ...
        issues.append(AuditIssue(
            module="Position Sizing", severity="P0", category="CRITICAL BUG",
...
def audit_risk_engine(report: AuditReport):
    ...
    try:
        ...
        if hasattr(re, '_risk_limits_loaded'):
            ...
            issues.append(AuditIssue(
                module="Risk Engine", severity="P1", category="SECURITY/SAFETY BUG",
        ...
        expected_limits = [
            "max_position_pct", "max_sector_concentration",
        ...
        for limit_name in expected_limits:
            if hasattr(re, '_risk_limits') and limit_name in re._risk_limits:
                ...
                issues.append(AuditIssue(
                    module="Risk Engine", severity="P2", category="MISSING FEATURE",
        ...
        if hasattr(re, '_risk_limits') and 'kill_switch' in re._risk_limits:
            ...
            issues.append(AuditIssue(
                module="Risk Engine", severity="P1", category="MISSING FEATURE",
        ...
        issues.append(AuditIssue(
            module="Risk Engine", severity="P1", category="CRITICAL BUG",
...
def audit_execution(report: AuditReport):
    ...
    try:
        ...
        if hasattr(es, 'execute') or hasattr(es, 'simulate'):
            ...
            issues.append(AuditIssue(
                module="Execution", severity="P2", category="MISSING FEATURE",
        ...
        issues.append(AuditIssue(
            module="Execution", severity="P1", category="CRITICAL BUG",
...
def audit_portfolio(report: AuditReport):
    ...
    try:
        ...
        if hasattr(pm, 'get_equity') or hasattr(pm, 'calculate_equity'):
            ...
            issues.append(AuditIssue(
                module="Portfolio", severity="P1", category="MISSING FEATURE",
        ...
        if hasattr(pm, 'apply_commission') or hasattr(pm, '_apply_commission'):
            ...
            issues.append(AuditIssue(
                module="Portfolio", severity="P2", category="MISSING FEATURE",
        ...
        issues.append(AuditIssue(
            module="Portfolio", severity="P1", category="CRITICAL BUG",
...
def audit_performance(report: AuditReport):
    ...
    try:
        ...
        if abs(cagr - total_return) < 0.001:
            issues.append(AuditIssue(
                module="Performance", severity="P1", category="FINANCIAL MATH BUG",
        ...
        if sharpe < -5 or sharpe > 5:
            issues.append(AuditIssue(
                module="Performance", severity="P2", category="FINANCIAL MATH BUG",
        ...
        issues.append(AuditIssue(
            module="Performance", severity="P1", category="CRITICAL BUG",
    ...
    try:
        ...
        if xu100_data is not None and not xu100_data.empty:
            ...
            if xu100_data.index.duplicated().any():
                issues.append(AuditIssue(
                    module="Benchmark", severity="P1", category="DATA BUG",
            ...
            issues.append(AuditIssue(
                module="Benchmark", severity="P0", category="CRITICAL BUG",
        ...
        issues.append(AuditIssue(
            module="Benchmark", severity="P1", category="CRITICAL BUG",
...
def audit_walk_forward(report: AuditReport):
    ...
    try:
        ...
        for i, fold in enumerate(splits[:3]):
            ...
            if train_dates and test_dates:
                ...
                if last_train >= first_test:
                    issues.append(AuditIssue(
                        module="Walk Forward", severity="P0", category="LEAKAGE RISK",
                    ...
                    if gap < 5:
                        issues.append(AuditIssue(
                            module="Walk Forward", severity="P1", category="LEAKAGE RISK",
        ...
        issues.append(AuditIssue(
            module="Walk Forward", severity="P1", category="CRITICAL BUG",
...
def audit_quality_gate(report: AuditReport):
    ...
    try:
        ...
        issues.append(AuditIssue(
            module="Quality Gate", severity="P1", category="CRITICAL BUG",
...
def audit_self_learning(report: AuditReport):
    ...
    try:
        ...
        if hasattr(continuous_learning, '_analyze_degradation') or hasattr(continuous_learning, 'analyze'):
            ...
            issues.append(AuditIssue(
                module="Self Learning", severity="P2", category="MISSING FEATURE",
        ...
        issues.append(AuditIssue(
            module="Self Learning", severity="P1", category="CRITICAL BUG",
    ...
    try:
        ...
        try:
            ...
            if result is None or (hasattr(result, 'empty') and result.empty):
                ...
                issues.append(AuditIssue(
                    module="Fail Safe", severity="P1", category="SECURITY/SAFETY BUG",
        ...
        issues.append(AuditIssue(
            module="Fail Safe", severity="P1", category="CRITICAL BUG",
...
def audit_survivorship(report: AuditReport):
    ...
    try:
        ...
        if hasattr(bist_universe, 'get_historical_universe') or hasattr(bist_universe, 'historical'):
            ...
            issues.append(AuditIssue(
                module="Survivorship", severity="P1", category="LEAKAGE RISK",
        ...
        if hasattr(bist_universe, 'delisted_tickers') or hasattr(bist_universe, 'DELISTED'):
            ...
            issues.append(AuditIssue(
                module="Survivorship", severity="P2", category="MISSING FEATURE",
        ...
        issues.append(AuditIssue(
            module="Survivorship", severity="P1", category="CRITICAL BUG",
...
def audit_lookahead(report: AuditReport):
    ...
    try:
        ...
        mask2 = tm.compute_mask("TEST", df_modified['Open'].values, df_modified['High'].values,
                               df_modified['Low'].values, df_modified['Close'].values, df_modified['Volume'].values)
                ...
                if isinstance(v1, np.ndarray) and isinstance(v2, np.ndarray) and len(v1) == len(v2):
                    ...
                    if len(v1) > 1:
                        ...
                        if diff > 0.001:
                            ...
                            issues.append(AuditIssue(
                                module="Look-Ahead", severity="P0", category="LEAKAGE RISK",
        ...
        issues.append(AuditIssue(
            module="Look-Ahead", severity="P1", category="CRITICAL BUG",
    ...
    try:
        ...
        if settings.is_production:
            if not settings.secret_key or len(settings.secret_key) < 16:
                issues.append(AuditIssue(
                    module="Security", severity="P0", category="SECURITY/SAFETY BUG",
        ...
        issues.append(AuditIssue(
            module="Security", severity="P2", category="SECURITY/SAFETY BUG",
...
def main():
    ...
    for name, audit_func in audit_modules:
        try:
            ...
            report.add_module(name, "FAIL", [AuditIssue(
                module=name, severity="P0", category="CRITICAL BUG",
    ...
    report_data = {
        ...
        "modules": {name: {"status": m.status, "issues": len(m.issues), "details": m.details}
