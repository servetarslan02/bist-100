...
def test_opportunity_engine() -> Any:
    """Opportunity Discovery Engine testleri."""
    ...
    passed += 1
    logger.info(f"  ✓ Bull stock: {score_bull.opportunity_score:.1f}")
    ...
    passed += 1
    logger.info(f"  ✓ Bear stock: {score_bear.opportunity_score:.1f} ({score_bear.signal_direction})")

    # 3. Risk-adjusted score
    assert score_bull.risk_adjusted_score <= score_bull.opportunity_score
    passed += 1
    logger.info("  ✓ Risk-adjusted score")

    # 4. Decomposition
    assert len(score_bull.decomposition) == 10
    assert sum(score_bull.decomposition.values()) > 0
    passed += 1
    logger.info(f"  ✓ Decomposition: {score_bull.decomposition}")

    # 5. Evidence
    assert len(score_bull.evidence) > 0
    passed += 1
    logger.info(f"  ✓ Evidence: {score_bull.evidence}")
    ...
    passed += 1
    logger.info(f"  ✓ Signal: {score_bull.signal_type} {score_bull.signal_direction}")
    ...
    passed += 1
    logger.info(f"  ✓ Universe scan: {len(results)} stocks ranked")
    ...
    passed += 1
    logger.info(f"  ✓ Top opportunities: {len(top)} stocks")
    ...
    passed += 1
    logger.info(f"  ✓ Confidence: {score_bull.confidence:.2f}")
