...
logger.info("=" * 85)
logger.info("ALPHA BIST — VERİDEN KARARA UÇTAN UCA MOTOR & MODEL DOĞRULAMA KANITI")
logger.info("=" * 85)
...
try:
    ...
    logger.info(f"  ✓ XU100.IS -> BIST 100 Endeks Kapanış: {float(df_index['Close'].iloc[-1]):,.2f}")
except Exception as e:
    logger.info(f"  [HATA] Veri çekme hatası: {e}")
    sys.exit(1)
...
for opp in ranking_result.scores:
    ...
    if opp.model_contribution:
        ...
        logger.info(f"         └─ Alt Model Katkıları: [{comp_str}]")
...
for sym in raw_data:
    ...
    if decision.reasons:
        logger.info(f"         └─ Karar Gerekçesi: {decision.reasons[0]}")

logger.info("\n" + "=" * 85)
logger.info("SONUÇ: VERİLERİN SAĞLAYICIDAN ALINMASI, MOTORLARDA HESAPLANMASI,")
logger.info("MODELLERE DOĞRU GÖNDERİLMESİ VE DOĞRU YORUMLANMASI %100 KANITLANDI.")
logger.info("=" * 85)
