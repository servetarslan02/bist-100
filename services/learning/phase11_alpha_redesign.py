...
def build_offline_dataset() -> Any:
    ...
    for date in common_dates:
        ...
        try:
            ...
            xu_fwd_5d = (
                (xu100_close.iloc[idx + 5] / xu100_close.iloc[idx] - 1.0) if idx + 5 < len(xu100_close) else np.nan
            )
        except Exception as e:
            logger.debug("xu100_forward_return_failed", date=str(date), error=str(e))
            xu_fwd_5d = np.nan
        ...
        for tk in features_by_ticker:
            ...
            try:
                ...
                fwd_5d = (
                    (df_tk.iloc[curr_idx + 5]["close"] / df_tk.iloc[curr_idx]["close"] - 1.0)
                    if curr_idx + 5 < len(df_tk)
                    else np.nan
                )
            except Exception as e:
                logger.debug("ticker_forward_return_failed", ticker=tk, date=str(date), error=str(e))
                fwd_5d = np.nan

            if not np.isnan(fwd_5d):
                row = {"date": date, "ticker": tk, "regime": regime, "fwd_5d": fwd_5d, "xu_fwd_5d": xu_fwd_5d}
...
def walk_forward_offline(df, val_dates, feature_cols) -> Any:
    ...
    for step, val_d in enumerate(val_dates):
        ...
        # M1: Ranker (L0_raw)
        m1 = lgb.LGBMRanker(
            n_estimators=50, random_state=42, n_jobs=1, verbose=-1, objective="lambdarank", metric="ndcg"
        )
        train_df_sorted = train_df.sort_values("date")
        groups = train_df_sorted.groupby("date").size().values

        y_m1 = (
            train_df_sorted.groupby("date")["L0_raw"]
        ...
        y_m3 = (
            train_df_sorted.groupby("date")["L1_excess"]
            .transform(lambda x: pd.qcut(x, 5, labels=False, duplicates="drop"))
            .fillna(0)
            .astype(int)
        )
        ...
        y_m4 = (
            train_df_sorted.groupby("date")["L4_vol_adj_excess"]
            .transform(lambda x: pd.qcut(x, 5, labels=False, duplicates="drop"))
            .fillna(0)
            .astype(int)
        )
...
def analyze_models(res_df) -> Any:
    ...
    logger.info(
        f"{'Metric':<15} | {'M0 (Reg-Raw)':<12} | {'M1 (Rank-Raw)':<13} | {'M2 (Reg-Exc)':<12} | {'M3 (Rank-Exc)':<13} | {'M4 (Rank-Risk)':<14}"
    )
    logger.info("-" * 85)
    ...
    for m in models:
        bull_df[f"{m}_decile"] = bull_df.groupby("date")[m].transform(
    ...
    for m in models:
        ...
        try:
            ...
            logger.debug("decile_analysis_failed", metric=m, error=str(e))
