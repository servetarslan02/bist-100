...
class TestRankingModel:
    ...
    def test_feature_name_sync(self, sample_features):
        ...
        # Tüm rule weight isimleri sample_features'ta olmalı
        for name in ranking_model._rule_weights.keys():
    ...
    def test_rank_output(self, sample_features):
        ...
        scores = ranking_model.rank(features_map, "BULL")

        assert len(scores) == 2
        assert scores[0].score >= scores[1].score  # Sıralı
...
class TestIntegration:
    ...
    async def test_full_pipeline(self, sample_price_data, sample_features):
        ...
        # 2. Rejim tespit
        price_data = {"THYAO": sample_price_data}
        regime = regime_engine.detect_regime(price_data)
        assert "regime" in regime

        # 3. Sıralama
        features_map = {"THYAO": {**sample_features, **features}}
        scores = ranking_model.rank(features_map, regime["regime"])
        assert len(scores) > 0
        ...
        decision_input = DecisionInput(
            ...
            ml_confidence=scores[0].confidence / 100,
            news_sentiment=0.3,
            sector="Havacılık",
            market_cap=50000000000,
