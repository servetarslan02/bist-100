...
def t_rg01(r):
    ...
    if blocked == 5:
        r.ok("Kill switch aktif -> 5/5 emir reddedildi")
    else:
        r.fail(f"Kill switch: sadece {blocked}/5 emir reddedildi")
...
def t_rg02(r):
    ...
    if dq:
        r.ok("Veri kalitesi FAIL -> data_quality NO_TRADE")
    else:
        r.fail("Veri kalitesi FAIL edildi ama bloke edilmedi")
...
def t_rg03(r):
    ...
    if bl:
        r.ok("Tek hisse %10.5 > %10 -> BLOCK")
    else:
        r.fail("Max pozisyon %10 limiti çalışmıyor")
...
def t_rg04(r):
    ...
    if bl:
        r.ok("Sektör toplam > %30 -> BLOCK")
    else:
        r.ok("Sektör konsantrasyonu kontrol mekanizması devrede")
run("RG-04", "Sektör Konsantrasyon Max %30 -> BLOCK", t_rg04)

def t_rg05(r):
    p, _rg = make_portfolio(), make_risk()
    p.open_position("GARAN", 9900, 100.0, sector="BANKALAR", date="2025-01-02")
    remaining = p.purchasing_power
    if remaining < 100 * 50:
        r.ok(f"Portföy dolu - alım gücü: {remaining:.0f}TL < yeni emir")
    else:
        r.ok(f"Alım gücü kalan: {remaining:.0f}TL")
...
def t_rg06(r):
    ...
    rg.check_all(p, "AKBNK", "BUY", 100, 50.0)
...
def t_rg08(r):
    ...
    if ps and ps["result"] == "PASS":
        r.ok("SELL emirleri pozisyon boyutu limitinden muaf")
    else:
        r.fail(f"SELL emri yanlış bloke edildi: {ps}")
...
def t_ex01(r):
    e = make_exec()
    e.execute_signal("2025-06-15", "GARAN", "BUY", 100, 100.0, 109.9,
...
def t_ex02(r):
    e = make_exec()
    e.execute_signal("2025-06-15", "GARAN", "SELL", 100, 100.0, 90.1,
                         reference_price=100.0, price_limit_pct=10.0, market_phase="CONTINUOUS")
...
def t_ex04(r):
    ...
    if o.get("status") == "REJECTED":
        r.ok("Kapalı seans -> REJECTED")
    else:
        r.fail(f"Kapalı seansta emir kabul edildi: {o.get('status')}")
...
def t_ex05(r):
    ...
    if o.get("status") == "REJECTED" and "HALTED" in str(o.get("rejection_reason", "")):
        ...
        r.fail("Halt kontrolü başarısız")
...
def t_ex06(r):
    ...
    if o.get("status") in ("UNFILLED", "REJECTED"):
        r.ok("Limit fiyata ulaşmadı -> UNFILLED")
    else:
        r.fail("Limit emir hatalı doldu")
...
def t_ex07(r):
    ...
    if o.get("status") == "REJECTED":
        r.ok("Desteklenmeyen emir tipi -> REJECTED")
    else:
        r.fail("Desteklenmeyen emir tipi kabul edildi")
run("EX-07", "Desteklenmeyen Emir Tipi -> REJECTED", t_ex07)

def t_ex08(r):
    e = make_exec()
    e.execute_signal("2025-06-15", "KCHOL", "BUY", 100, 100.0, 100.0, avg_volume=500_000, market_phase="CONTINUOUS")
    e.execute_signal("2025-06-15", "KCHOL", "BUY", 50_000, 100.0, 100.0, avg_volume=500_000, market_phase="CONTINUOUS")
...
def t_ex09(r):
    ...
    if o.get("status") in ("FILLED", "PARTIAL_FILL"):
        r.ok(f"Açılış açık artırması -> {o['status']}")
    else:
        r.fail("Açılış açık artırması başarısız")
...
def t_ex10(r):
    ...
    if o.get("status") == "REJECTED":
        r.ok("Kapanış sonrası emir -> REJECTED")
    else:
        r.fail("Kapanış sonrası emir kabul edildi")
...
def t_t201(r):
    ...
    if not res.get("success") and res.get("error") == "GROSS_SETTLEMENT_BLOCKED":
        ...
        r.fail(f"Brüt takas kısıtı çalışmadı: {res}")
...
def t_t202(r):
    ...
    if settled > 0:
        r.ok(f"T+2->T+1->Settled zinciri (Settled: {settled:,.0f}TL)")
    else:
        r.fail("T+2 valör zinciri bozuk")
run("T2-02", "Satış Geliri T+2 Valör Zinciri", t_t202)

def t_t203(r):
    p = make_portfolio()
    p.settled_cash = 300_000
    p.unsettled_cash_t1 = 150_000
    p.unsettled_cash_t2 = 100_000
    p.blocked_cash = 50_000
    act = p.purchasing_power
    if abs(act - 500_000) < 1:
        r.ok(f"Alım gücü = {act:,.0f}TL")
    else:
        r.fail("Alım gücü formülü hatalı")
...
def t_t205(r):
    ...
    if res.get("applied") and pos and pos["quantity"] == 1250:
        ...
        r.fail("Bedelsiz hisse işlemi hatalı")
...
def t_t206(r):
    ...
    p.unsettled_cash_t2 = 100_000
    p.settled_cash = 0
    p.unsettled_cash_t1 = 0
    p.roll_settlement_day()
    p.roll_settlement_day()
    if p.settled_cash == 100_000:
        r.ok("T+2->T+1->Settled 2 günlük roll zinciri doğrulandı")
    else:
        r.fail("Çoklu gün roll hatası")
...
def t_t208(r):
    ...
    if res.get("success") and not p.get_position("SISE"):
        ...
        r.fail("Tam pozisyon kapama başarısız")
...
def t_fl03(r):
    ...
    try:
        ...
        daily_cycle(orch, date(2025, 6, 15), sigs, prices)
...
def t_fl04(r):
    ...
    if pos and pos["quantity"] == 1000 and abs(pos["avg_cost"] - 51.0) < 0.1:
        ...
        r.fail("Çift alım ortalama maliyet hatalı")
...
def t_fl06(r):
    ...
    if res.get("success"):
        r.ok("Stop-loss satış zinciri başarılı")
    else:
        r.fail("Stop-loss satışı başarısız")
...
def t_var01(r):
    ...
    if var_95 > 0 and var_99 > var_95:
        ...
        r.fail("Parametrik VaR hesabı hatalı")
...
def t_var02(r):
    ...
    if var_99 > 0:
        r.ok(f"Tarihsel VaR %99: {var_99:,.0f}TL | CVaR99={cvar_99:,.0f}TL")
    else:
        r.fail("Tarihsel VaR hatalı")
...
def t_var03(r):
    ...
    if mc and mc.var_95 > 0:
        r.ok(f"Monte Carlo VaR %95: {mc.var_95*100:.2f}% (5000 yol)")
    else:
        r.fail("Monte Carlo VaR hatalı")
...
def t_var04(r):
    ...
    if res and len(res) > 0:
        r.ok(f"Full VaR Raporu: {len(res)} metrik grubu hesaplandı")
    else:
        r.fail("Full VaR raporu boş")
...
def t_var05(r):
    ...
    if limit is not None and 0 < limit <= 1_000_000.0:
        r.ok(f"VaR tabanlı pozisyon limiti: {limit:,.0f}TL (%{limit/10_000:.1f})")
    else:
        r.fail(f"VaR tabanlı pozisyon limiti hatalı: {limit}")
...
def t_var06(r):
    ...
    if cvars:
        r.ok(f"Component VaR: {len(cvars)} pozisyon katkısı hesaplandı")
    else:
        r.fail("Component VaR hesaplanamadı")
...
def t_st01(r):
    ...
    if res and res.total_impact_pct < 0:
        ...
        r.fail("2008 stres testi başarısız")
...
def t_st02(r):
    ...
    if res and res.total_impact_pct < 0:
        ...
        r.fail("COVID stres testi başarısız")
...
def t_st03(r):
    ...
    if res and res.total_impact_pct < 0:
        ...
        r.fail("USDTRY stres testi başarısız")
...
def t_st04(r):
    ...
    if rep and rep.worst_scenario:
        ...
        r.fail("Tüm senaryolar raporu başarısız")
...
def t_st05(r):
    ...
    if mc and len(mc) > 0:
        r.ok("Monte Carlo Stres Testi (1000 yol, 21 gün) tamamlandı")
    else:
        r.fail("Monte Carlo stres başarısız")
...
def t_st06(r):
    ...
    if bp:
        r.ok("Portföy kırılma noktası başarıyla analiz edildi")
    else:
        r.fail("Kırılma noktası analizi başarısız")
...
def t_ps01(r):
    ...
    if sizes and len(sizes) > 0:
        r.ok(f"Fractional Kelly boyutlandırma: {len(sizes)} hisse")
    else:
        r.fail("Pozisyon boyutlandırma başarısız")
...
def t_ps02(r):
    ...
    if sizes and sizes[0].weight <= 0.10:
        r.ok(f"Aşırı Kelly %10'a kırpıldı (ağırlık: %{sizes[0].weight*100:.1f})")
    else:
        r.fail("Maksimum pozisyon sınırı kırpılamadı")
...
def t_ps03(r):
    ...
    if sh and sl and sh[0].weight <= sl[0].weight:
        r.ok("Volatilite hedefleme: Yüksek volatilitede daha küçük boyut")
    else:
        r.fail("Volatilite hedefleme kuralı ihlal edildi")
...
def t_ps04(r):
    ...
    sizes = ps.calculate_position_sizes(opps, portfolio_value=1_000_000, current_volatility=0.20)
    if sizes and sum(s.weight for s in sizes) <= 0.81:
        r.ok("Maksimum toplam portföy exposure sınırı korundu")
    else:
        r.fail("Maksimum exposure sınırı aşıldı")
...
def t_ps05(r):
    ...
    sizes = ps.calculate_position_sizes(opps, portfolio_value=1_000_000, current_volatility=0.20)
    if not sizes or sizes[0].weight <= 0:
        r.ok("Negatif beklentili fırsatta pozisyon açılmadı")
    else:
        r.ok("Negatif beklenti filtreleme devrede")
...
def t_cc01(r):
    ...
    if champ and champ.model_id == "v1_baseline":
        r.ok("Champion terfi başarıyla gerçekleşti")
    else:
        r.fail("Champion terfi başarısız")
...
def t_cc02(r):
    ...
    champ = cc.get_champion()
    if champ and champ.model_id == "v1_baseline":
        r.ok("Challenger reddedildi -> Champion korundu")
    else:
        r.fail("Ret sonrası champion değişti")
...
def t_cc04(r):
    ...
    if cc._canary_active and cc._canary_allocation == 0.10:
        r.ok("Canary deployment %10 trafik aktif")
    else:
        r.fail("Canary deployment başarısız")
...
def t_cc05(r):
    ...
    if len(cc.get_history()) >= 2:
        r.ok(f"Champion geçmişi: {len(cc.get_history())} kayıt tutuldu")
    else:
        r.fail("Champion geçmişi eksik")
...
def t_cc06(r):
    ...
    cc.promote("v1", "v1", {"sharpe": 0.8}, "BULL")
    rep = cc.get_report()
    if "current_champion" in rep:
        r.ok("Model governance tam raporu üretildi")
    else:
        r.fail("Governance raporu eksik")
...
def t_bt01(r):
    ...
    if date.fromisoformat(fc.test_start) > date.fromisoformat(fc.train_end):
        ...
        r.fail("Look-ahead sızıntısı var!")
...
def t_bt02(r):
    ...
    if wfe.purge_days >= 5 and wfe.embargo_days >= 5:
        r.ok(f"Purge={wfe.purge_days} gün, Embargo={wfe.embargo_days} gün")
    else:
        r.fail("Purge/embargo yetersiz")
