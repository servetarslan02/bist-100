...
TICKERS = [
    ...
    "PGSUS","TAVHL","LOGO","CCOLA","MPARK","CIMSA","AKCNS",
    "VESTL","VESBE","BRSAN","BURCE","ISDMR",
    "TKFEN","ENKAI","PETKM","AGHOL","AEFES",
    "TSKB","KLNMA","ISGYO","ALGYO","ULKER","BANVT",
    "KNFRT","BIZIM","IHLAS","MAVI","PKART",
    "BRISA","JANTS","GUBRF","AFYON","ADEL",
]
raw = yf.download([f"{t}.IS" for t in TICKERS], start="2019-01-01", end=HOLDOUT,
...
def maxdd(s): c=(1+s).cumprod(); return (c/c.cummax()-1).min()*100

def momentum_top(prices, returns, lb, top_n, skip=21):
...
for yr in years:
    ...
    if py.empty or len(py)<20: continue
    ...
    def sc(s): return f"%{cagr(s):+.0f}" if len(s)>5 else "N/A"
