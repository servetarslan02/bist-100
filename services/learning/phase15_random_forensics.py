...
class FastPortfolioEngine:
    ...
    def run(self, eval_dates, cached_scores, filter_seed=None, shuffle_scores=False, random_selection=False):
        ...
        for current_date in eval_dates:
            ...
            if random_selection:
                ...
                if shuffle_scores:
                    ...
                    random.shuffle(vals)
                    scores = {tk: v for tk, v in zip(scores.keys(), vals)}
            ...
            for tk, pos in list(positions.items()):
                ...
                if pnl_pct <= -5.0 or pnl_pct >= 12.0 or pos["days_held"] >= 5:
                    ...
                    trade_log.append({
                        ...
                        "pnl": net_trade_pnl, "pnl_pct": pnl_pct
                    })
            ...
            if permit_long:
                ...
                if open_slots > 0 and portfolio_cash > 200_000:
                    target_alloc_per_slot = min(portfolio_cash / open_slots, (portfolio_cash + sum(p["shares"] * self.features[t].loc[current_date]["close"] for t, p in positions.items())) * 0.20)
                    for cand in top_candidates[:open_slots]:
                        ...
                        if shares > 0:
                            ...
                            positions[cand["ticker"]] = {"shares": shares, "entry_price": cand["close_price"], "entry_date": current_date, "days_held": 0}

            current_equity = portfolio_cash + sum(p["shares"] * float(self.features[t].loc[current_date]["close"]) for t, p in positions.items())
...
def calculate_metrics(eq_curve, trade_log):
    init = 10_000_000.0
    final = eq_curve[-1]
