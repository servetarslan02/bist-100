...
class CodeInspector(ast.NodeVisitor):
    ...
    def _check_forbidden_imports(self, mod_name: str, lineno: int) -> Any:
        ...
        is_prod = (
            ...
            or self.rel_path.startswith("workers")
        )
...
def scan_source_text(rel_path: str, content: str) -> list[AuditFinding]:
    ...
    hardcoded_secret_pattern = re.compile(
        r'(password|secret|api_key|token)\s*=\s*["\'][a-zA-Z0-9_\-]{8,}["\']', re.IGNORECASE
    )

    for idx, line in enumerate(lines, 1):
        # Görev işareti kontrolü
        ...
        if not rel_path.startswith("tests") and not rel_path.endswith((".example", ".sample")):
            ...
            if m_sec and not any(
                safe in line.lower() for safe in ("os.getenv", "settings.", "environ", "dummy", "test")
            ):
                findings.append(
                    AuditFinding(
...
def run_cross_audit() -> Any:
    ...
    logger.info("🚀 ALPHA BIST 360° Derin Çapraz Denetim Motoru Başlatılıyor...")
    ...
    logger.info(f"🔍 Toplam taranacak Python dosyası: {total_files}")
    ...
    logger.info(f"📊 Toplam Satır: {total_lines:,} | Toplam Dosya: {total_files}")
    logger.info(f"🔴 Kritik Sorun: {crit_count} | 🟠 Yüksek: {high_count} | 🟡 Orta: {med_count}")
    logger.info(f"🛡️ Genel Sistem Sağlık Skoru: {health_score} / 100\n")
    ...
    with open(md_path, "w", encoding="utf-8") as mf:
        ...
        mf.write(
            f"| **EMPTY_CODE** | 🔴 KRİTİK | {cat_counts.get('EMPTY_CODE', 0)} | İçi boş / tamamlanmamış fonksiyonlar |\n"
        )
        mf.write(
            f"| **FAIL_CLOSED** | 🔴 KRİTİK | {cat_counts.get('FAIL_CLOSED', 0)} | Hata yutan `except: pass` blokları |\n"
        )
        mf.write(f"| **SYNTAX_ERROR** | 🔴 KRİTİK | {cat_counts.get('SYNTAX_ERROR', 0)} | Bozuk Python dosyaları |\n")
        mf.write(
            f"| **MOCK_LEAK** | 🔴 KRİTİK | {cat_counts.get('MOCK_LEAK', 0)} | Üretim servisinde mock kalıntıları |\n"
        )
        mf.write(
            f"| **ASYNC_BLOCKING** | 🟠 YÜKSEK | {cat_counts.get('ASYNC_BLOCKING', 0)} | Async içinde blocking çağrılar (`time.sleep` vb.) |\n"
        )
        mf.write(
            f"| **PANDAS_VIOLATION** | 🟠 YÜKSEK | {cat_counts.get('PANDAS_VIOLATION', 0)} | Polars yerine yasaklı Pandas kullanımı |\n"
        )
        mf.write(
            f"| **FAKE_TEST** | 🟠 YÜKSEK | {cat_counts.get('FAKE_TEST', 0)} | `assert ... or True` sahte testleri |\n"
        )
        mf.write(
            f"| **TODO_MARKER** | 🟡 ORTA | {cat_counts.get('TODO_MARKER', 0)} | Kod içinde unutulmuş TODO/FIXME'ler |\n\n"
        )
    ...
    logger.info(f"📄 Markdown Raporu Kaydedildi: {md_path}")
    logger.info(f"💾 JSON Verisi Kaydedildi: {json_path}")
