...
def find_files_with_prints() -> Any:
    """Otomatik eklendi."""
    files = []
...
def ensure_imports(content) -> Any:
...
def extract_balanced(text, start) -> Any:
...
logger.info(f"Found {len(files)} files with print()")

for fp in files:
    rel = os.path.relpath(fp, SERVICES)
    n = convert_file(fp)
    if n > 0:
        logger.info(f"  ✓ {rel}: {n} converted")
        total += n
    else:
        logger.info(f"  - {rel}: no changes")
