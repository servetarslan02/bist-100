...
def run_feature_discovery() -> Any:
    ...
    for d in val_dates:
        ...
        if r == "BULL_TREND":
            if days_in_regime <= 20:
                fine_regime = "EARLY_BULL"
            else:
                fine_regime = "LATE_BULL"
    ...
    logger.info(
        "-> Teşhis: roc_20d ve momentum_20d %100 duplicate! price_vs_sma20 ve bb_position çok yüksek korelasyonlu."
    )
    ...
    for f in feature_cols:
        if f == "momentum_20d":
            continue  # skip duplicate in print
        ...
        logger.info(
            f"{f:15} | 1D: {ic_1:>6.3f} | 5D: {ic_5:>6.3f} | 10D: {ic_10:>6.3f} | 20D: {ic_20:>6.3f} | ICIR: {icir:>5.2f} | Pos%: {pos_rate:>4.1f}%"
        )
    ...
    for f in feature_cols:
        ...
        logger.info(
            f"{f:15} | Q1(Low): %{q1:>5.2f} | Q2: %{q2:>5.2f} | Q3: %{q3:>5.2f} | Q4: %{q4:>5.2f} | Q5(High): %{q5:>5.2f}"
        )
    ...
    logger.info(
        "- atr_pct, volatility_20d: A) ROBUST INFORMATION (Ters yönlü - Düşük volatilite yüksek getiri üretiyor, Low-Vol anomaly)"
    )
