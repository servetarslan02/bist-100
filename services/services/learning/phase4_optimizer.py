    ...
    for step_i, current_date in enumerate(eval_dates):
        ...
        for tk, pos in list(positions.items()):
            ...
            if candidate_name == "C_Adaptive_Stop":
                if regime_tag in ["BULL_TREND", "V_DIP_RECOVERY"]: atr_mult = 3.0
                elif regime_tag == "BEAR_MARKET": atr_mult = 1.5
                else: atr_mult = 2.0
                # Tighten further if profit > 8%
                if pnl_pct > 8.0: atr_mult *= 0.6
        ...
        max_pos = 6 if (candidate_name == "D_Full_Deployment" and regime_tag in ["BULL_TREND", "V_DIP_RECOVERY"]) else FROZEN_PARAMS["max_pos"].get(current_regime, 2)
        ...
        if len(equity_curve) > 1: daily_rets.append(equity_curve[-1] / equity_curve[-2] - 1.0)
