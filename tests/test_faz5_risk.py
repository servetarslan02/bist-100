...
def test_ledoit_wolf():
    ...
    passed += 1
    print(f"  ✓ Auto shrinkage: {ledoit_wolf._estimate_shrinkage(returns, cov, np.diag(np.diag(cov))):.3f}")
...
def test_kelly_criterion():
    ...
    passed += 1
    print(f"  ✓ Half-Kelly: {kelly_half:.4f} < Full: {kelly_full:.4f}")
...
def test_rebalance():
    ...
    passed += 1
    print(f"  ✓ Large deviation: {len(orders)} orders")
    ...
    passed += 1
    print(f"  ✓ Turnover limit: {total_change:.1f}%")
...
def test_concentration_risk():
    ...
    passed += 1
    print(f"  ✓ Equal weight HHI: {hhi:.4f}")
    ...
    assert hhi_conc > hhi
    passed += 1
    print(f"  ✓ Concentrated HHI: {hhi_conc:.4f}")
    ...
    passed += 1
    print(f"  ✓ Sector concentration: {sector_conc}")
    ...
    passed += 1
    print(f"  ✓ Max concentration: {max_ticker}={max_weight}")
...
def main():
    print("=" * 60)
    print("  FAZ 5 — Risk & Portfolio Test Suite")
    print("=" * 60)
