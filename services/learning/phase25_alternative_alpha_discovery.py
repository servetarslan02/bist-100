...
def evaluate_signal(df_liquid, signal_col) -> Any:
    ...
    for d, grp in df_liquid.groupby("date"):
        ...
        # Null Spread
        shuf_q = (
            pd.qcut(grp[signal_col].sample(frac=1).values, 5, labels=False, duplicates="drop")
            if q is not None
            else None
        )
    ...
    metrics = {}
    metrics["ic_1d"] = res_df["ic_1d"].mean()
    metrics["ic_5d"] = res_df["ic_5d"].mean()
    metrics["ic_10d"] = res_df["ic_10d"].mean()
    metrics["ic_20d"] = res_df["ic_20d"].mean()
    metrics["icir_5d"] = (metrics["ic_5d"] / res_df["ic_5d"].std()) * np.sqrt(252)

    metrics["t5_spr"] = res_df["t5_spr"].mean()
    for i in range(5):
        metrics[f"Q{i + 1}"] = res_df[f"Q{i + 1}"].mean() if f"Q{i + 1}" in res_df.columns else 0
    ...
    metrics["ci_L"] = np.percentile(boot, 2.5)
    metrics["ci_U"] = np.percentile(boot, 97.5)
    ...
    # Regimes
    metrics["regimes"] = {
        reg: res_df[res_df["regime"] == reg]["t5_spr"].mean()
...
def run_phase_25() -> Any:
    ...
    for sig in signals:
        ...
        logger.info("1. IC (Rank Correlation)")
        logger.info(
            f"   1D: {m['ic_1d']:7.4f} | 5D: {m['ic_5d']:7.4f} (ICIR: {m['icir_5d']:5.2f}) | 10D: {m['ic_10d']:7.4f}"
        )

        logger.info("2. Q1-Q5 MONOTONICITY (5D Spread)")
        logger.info(
            f"   Q1:%{m['Q1']:6.3f} | Q2:%{m['Q2']:6.3f} | Q3:%{m['Q3']:6.3f} | Q4:%{m['Q4']:6.3f} | Q5:%{m['Q5']:6.3f}"
        )

        logger.info("3. NULL & STATISTICAL SIGNIFICANCE")
        ...
        logger.info("4. STABILITY")
        logger.info(f"   Time Blocks   : {[round(x, 3) for x in m['blocks_spr']]}")
        logger.info(
            f"   Regimes       : ER_BULL:%{m['regimes'].get('EARLY_BULL', 0):.3f} | LT_BULL:%{m['regimes'].get('LATE_BULL', 0):.3f} | BEAR:%{m['regimes'].get('BEAR_MARKET', 0):.3f}"
        )

        logger.info("5. CONCENTRATION")
