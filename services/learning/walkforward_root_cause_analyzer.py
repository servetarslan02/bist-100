...
def run_root_cause_analysis() -> Any:
    ...
    logger.info(
        "| Model | Sinyal Oynaklığı (Std) | Günlük Sinyal Değişim Hızı (|Δs|) | 1-Günlük Otokorelasyon | Değerlendirme |"
    )
    logger.info("|---|---|---|---|---|")
    for m in models:
        ...
        status = (
            "🔴 Çok Oynak (Churn Kaynağı)"
            if daily_diff > 0.25
            else ("🟡 Orta" if daily_diff > 0.12 else "🟢 Stabil Trend")
        )
    ...
    logger.info(
        "5 gün sonra zorla kapatılan kârlı pozisyonların 10, 20 ve 40 gün sonraki gerçek getirileri inceleniyor..."
    )
    ...
    logger.info(
        "  💡 TESPİT: 5. günde zorla pozisyon kapatmak, BIST'teki büyük trend dalgalarını (20-40 günlük rallileri) %70 oranında kaçırıyor!"
    )

    # 3. SIDEWAYS ve HIGH_VOLATILITY Kaybının Temel Sebebi
    logger.info("\n[3] SIDEWAYS VE HIGH_VOLATILITY KANAMA ANALİZİ:")
    logger.info(
        "  • SIDEWAYS (Yatay): Hisse Bollinger üst bandına çarpıp geri döndüğünde model 5 günde 2 kez yön değiştiriyor."
    )
    logger.info(
        "  • HIGH_VOLATILITY: Geniş spread ve gap açılışları %0.05 slippage ve %5 stop-loss tetiklenmesiyle sermayeyi eritiyor."
    )
    logger.info(
        "  • Gereksiz Histeresis Yokluğu: Model skoru 0.12'den 0.14'e çıktığında portföy mevcut hisseyi satıp yeni hisse alıyor ve her seferinde %0.124 sürtünme ödüyor."
    )
