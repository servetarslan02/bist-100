...
def test_pit_store() -> Any:
    ...
    passed += 1
    logger.info("  ✓ PIT Store: insert, get_as_of, revision")

    # 2. Latest
    latest = store.get_latest("THYAO", "pe_ratio")
    assert latest == 9.0
    passed += 1
    logger.info("  ✓ PIT Store: get_latest")
    ...
    passed += 1
    logger.info(f"  ✓ PIT Store: history ({len(history)} revisions)")

    # 4. Bulk insert
    store.bulk_insert("ASELS", {"price": 38.5, "volume": 500000}, day1, "yfinance")
    assert store.get_latest("ASELS", "price") == 38.5
    passed += 1
    logger.info("  ✓ PIT Store: bulk_insert")

    # 5. Stats
    stats = store.get_stats()
    assert stats["tickers"] >= 2
    passed += 1
    logger.info(f"  ✓ PIT Store: stats ({stats})")

    return passed, failed


def test_cross_source_reconciliation() -> Any:
    ...
    passed += 1
    logger.info(f"  ✓ Consistent: value={result.value}, quality={result.quality_score}")
    ...
    passed += 1
    logger.info(f"  ✓ Discrepancy: {result2.discrepancy_pct}%, consistent={result2.is_consistent}")
    ...
    passed += 1
    logger.info(f"  ✓ Anomaly: discrepancy={result3.discrepancy_pct:.1f}%, quality={result3.quality_score}")

    # 4. Tek kaynak
    result4 = rec.reconcile_price({"yfinance": 305.25})
    assert result4.confidence < 0.8  # Tek kaynak düşük güven
    passed += 1
    logger.info(f"  ✓ Single source: confidence={result4.confidence}")
    ...
    passed += 1
    logger.info(f"  ✓ Best source: {result5.source}")

    # 6. Price jump detection
    is_jump, change = rec.detect_price_jump("TEST", 120, 100, 0.25)
    assert abs(change - 20.0) < 0.1
    passed += 1
    logger.info(f"  ✓ Price jump: {change:.1f}%")

    return passed, failed


def test_streaming_anomaly() -> Any:
    ...
    passed += 1
    logger.info(f"  ✓ Normal price: anomaly={result.is_anomaly}")
    ...
    passed += 1
    logger.info(f"  ✓ Price jump: anomaly={result.is_anomaly}, severity={result.severity}")
    ...
    assert result.is_anomaly
    passed += 1
    logger.info(f"  ✓ Volume anomaly: zscore={result.zscore}")

    # 4. Spread anomalisi
    result = detector.check_spread("TEST", 100.0, 106.0)
    assert result.is_anomaly  # %6 spread
    passed += 1
    logger.info(f"  ✓ Spread anomaly: {result.details}")
    ...
    passed += 1
    logger.info(f"  ✓ All checks: {len(anomalies)} anomalies")
...
def main() -> Any:
    """Otomatik eklendi."""
    logger.info("=" * 60)
    logger.info("  Bölüm 1 & 2 — Enhancement Tests")
    logger.info("=" * 60)
