...
BIST_TEST_STOCKS = [
    ...
    "EREGL.IS", "BIMAS.IS", "FROTO.IS", "PGSUS.IS", "SISE.IS",
    "AKBNK.IS", "YKBNK.IS", "TCELL.IS", "SAHOL.IS", "ENKAI.IS"
]
...
def run_benchmark():
    ...
    print(f"{'Kazanma Oranı (Win Rate)':<30} | %{s_base['win_rate']:<21} | %{s_adv['win_rate']:<21} (ARTIŞ: +%{s_adv['win_rate'] - s_base['win_rate']:.1f})")
    ...
    if s_adv['win_rate'] > s_base['win_rate'] and s_adv['profit_factor'] > s_base['profit_factor']:
        print("✅ KANITLANDI: 10/10 Mum ve Price Action Motoru, eski sisteme kıyasla hem Kazanma Oranında hem de Kar Çarpanında belirgin üstünlük sağladı!")
    print("=" * 80)
