...
def backtest(t1, t2, hr, pdata, rdata, lookback=60) -> Any:
    """
    ...
    Short spread = short t1 / long t2
      gunluk PnL = -(r1 - hr*r2)
    """
    if t1 not in pdata.columns or t2 not in pdata.columns:
        return None
    p1 = pdata[t1].dropna()
    p2 = pdata[t2].dropna()
    r1 = rdata[t1]
...
for r in results:
    ...
    logger.info(
        f"   {r['pair']:<16} {r['oos_cagr']:>8}% {r['oos_sh']:>6} {r['oos_dd']:>7}% {r['full_cagr']:>6}% {r['full_sh']:>7} {r['oos_n']:>6} %{r['oos_wr']:>4}{tag}"
    )
...
for r in top:
    ...
    if t1 not in prices_full.columns or t2 not in prices_full.columns:
        continue
    p1 = prices_full[t1].dropna()
    p2 = prices_full[t2].dropna()
    r1v = returns_full[t1]
    r2v = returns_full[t2]
    idx = p1.index.intersection(p2.index)
    p1v, p2v = p1[idx].values, p2[idx].values
    r1a = r1v.reindex(idx).fillna(0).values
    r2a = r2v.reindex(idx).fillna(0).values
    lookback = 60
    pos = 0
    pnls = []
    for i in range(lookback, len(p1v)):
        ...
        pnls.append(d)
    if pnls:
        port_daily.append(pd.Series(pnls, index=p1[idx].index[lookback:]))
...
if port_daily and top:
    ...
    logger.info("\n   ╔═══════════════════════════════════════╗")
    logger.info("   ║  PORTFOY (Pozitif Sharpe'li ciftler) ║")
