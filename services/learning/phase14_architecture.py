...
if __name__ == "__main__":
    ...
    eq_m0, tr_m0, gp_m0, gl_m0, exp_m0 = run_simulation(
        trainer_m0, val_dates, features_by_ticker, xu100_close, filter_mode="NONE"
    )

    # 2. M1
    trainer_ranker = ModelTrainerRanker(feature_cols)
    eq_m1, tr_m1, gp_m1, gl_m1, exp_m1 = run_simulation(
        trainer_ranker, val_dates, features_by_ticker, xu100_close, filter_mode="NONE"
    )

    # 3. M2
    eq_m2, tr_m2, gp_m2, gl_m2, exp_m2 = run_simulation(
        trainer_ranker, val_dates, features_by_ticker, xu100_close, filter_mode="ACTUAL"
    )
    ...
    def print_metrics(name, eq_curve, trades, gp, gl, exp) -> Any:
        ...
        logger.info(
            f"{name:30} | CAGR: %{cagr:>6.2f} | MaxDD: %{mdd:>5.2f} | PF: {pf:>4.2f} | Trades: {trades:>4} | Avg Exposure: %{exp * 100:>4.1f}"
        )
        return cagr
    ...
    logger.info("==================================================")
    eq_lag, tr_lag, gp_lag, gl_lag, exp_lag = run_simulation(
        trainer_ranker, val_dates, features_by_ticker, xu100_close, filter_mode="LAGGED"
    )
    eq_rnd, tr_rnd, gp_rnd, gl_rnd, exp_rnd = run_simulation(
        trainer_ranker, val_dates, features_by_ticker, xu100_close, filter_mode="RANDOM"
    )
