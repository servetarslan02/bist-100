...
def run_robustness_test(eval_dates, features_by_ticker, xu100_close, trainer, breadth_mode="ACTUAL") -> Any:
    ...
    regime_pnl = {
        k: 0.0
        for k in ["BULL_TREND", "BEAR_MARKET", "SIDEWAYS_RANGE", "HIGH_VOLATILITY", "LOW_VOLATILITY", "V_DIP_RECOVERY"]
    }
...
if __name__ == "__main__":
    ...
    eq_act, ret_act, dd_act, tr_act, rpnl_act = run_robustness_test(
        val_dates, features_by_ticker, xu100_close, trainer, "ACTUAL"
    )
    logger.info(f"[C_ACTUAL]  Net: %{ret_act:>6.2f} | Max DD: %{dd_act:>5.2f} | PnL: ₺{sum(rpnl_act.values()):,.0f}")

    eq_lag, ret_lag, dd_lag, tr_lag, rpnl_lag = run_robustness_test(
        val_dates, features_by_ticker, xu100_close, trainer, "LAGGED_5D"
    )
    logger.info(
        f"[C_LAGGED]  Net: %{ret_lag:>6.2f} | Max DD: %{dd_lag:>5.2f} | PnL: ₺{sum(rpnl_lag.values()):,.0f} (Breadth delayed 5 days)"
    )

    eq_con, ret_con, dd_con, tr_con, rpnl_con = run_robustness_test(
        val_dates, features_by_ticker, xu100_close, trainer, "CONSTANT"
    )
    logger.info(
        f"[C_CONST]   Net: %{ret_con:>6.2f} | Max DD: %{dd_con:>5.2f} | PnL: ₺{sum(rpnl_con.values()):,.0f} (Breadth fixed at 50%)"
    )
    ...
    for k, v in rpnl_act.items():
        if v != 0:
            logger.info(f"{k.ljust(15)}: ₺{v:>10,.0f}")
    ...
    for i, chunk in enumerate(chunks):
        ...
        logger.info(
            f"Window {i + 1} ({chunk[0].strftime('%Y-%m')} to {chunk[-1].strftime('%Y-%m')}): Net Return %{chunk_ret:.2f}"
        )
