...
TR_TZ: timezone = timezone(timedelta(hours=3))

DEFAULT_CATCHUP_SCAN_LIMIT: int = 50
...
class MasterStartupCatchup:
    ...
    def __init__(self) -> None:
        """MasterStartupCatchup başlatıcı."""
    ...
    def get_missed_trading_days(self, last_date_str: str | None, current_date: date) -> list[date]:
        ...
        Args:
            last_date_str: En son çalıştırılan döngünün tarihi (ISO format veya None).
            current_date: Bugünün yerel tarihi.

        Returns:
            list[date]: Kronolojik sırayla kaçırılmış iş günleri listesi.
        """
    ...
    async def execute_full_catchup(self) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: Telafi döngüsü sonuç raporu (kaçırılan günler, backfill durumu,
                oynatılan işlemler, model eğitimi durumu).
        """
        logger.info("Master Startup Catch-up otonom telafi dongusu baslatiliyor")
        ...
        if missed_days:
            try:
                logger.info("Kacirilan seanslar icin piyasa verileri taraniyor (Backfill)")
                active_tickers = [p["ticker"] for p in paper_orchestrator.portfolio.get_all_positions()]
                from services.ingestion.bist_universe import bist_universe

                target_tickers = list(set(active_tickers + list(bist_universe.BIST_ALL_TICKERS)))
                gaps = await backfill_manager.detect_all_gaps(tickers=target_tickers)
                if gaps:
                    logger.info("Hedef veri boslugu tespit edildi, dolduruluyor", gap_count=len(gaps))
                    await backfill_manager.backfill_all(gaps)
                    results["data_backfilled"] = True
                else:
                    logger.info("Hedef hisselerin gecmis verileri tam")
            except Exception as e:
                logger.warning("Backfill sirasinda uyari, atlandi", error=str(e))
        else:
            logger.info("Kacirilan seans gunu bulunmuyor, veritabani guncel")

        # 3. Kaçırılan Seans Günlerini Sırasıyla Oynat (Replay & Execution)
        if missed_days:
            logger.info("Kacirilan seans gunleri isletiliyor", missed_count=len(missed_days))
            for day in missed_days:
                day_str = day.isoformat()
                logger.info("Seans telafisi isleniyor", date=day_str)
                try:
                    ...
                    logger.error("Seans telafisinde hata olustu", date=day_str, error=str(day_err))
            logger.info("Tum kacirilan seanslar telafi edildi ve portfoye islendi")
        else:
            logger.info("Kacirilan gecmis seans gunu yok")

        # 4. Yapay Zeka Model ve Öğrenme Telafisi (ML Catch-up)
        try:
            logger.info("Model drift ve eksik egitim kontrolu yapiliyor")
            ...
            if ml_res.get("status") != "up_to_date":
                ...
                logger.info("Modeller guncel, yeniden egitime gerek yok")
        except Exception as ml_err:
            logger.warning("ML telafi dongusunde uyari", error=str(ml_err))

        # 5. Anlık Durumu Eşitle ve Taze Sinyalleri Yayınla
        try:
            logger.info("Guncel portfoy ve sinyaller Redis'e senkronize ediliyor")
            fresh_signals = bist_ml_scanner.scan_all_opportunities(limit=DEFAULT_CATCHUP_SCAN_LIMIT)

            r_host = os.environ.get("REDIS_HOST", "redis")
            r_port = int(os.environ.get("REDIS_PORT", DEFAULT_REDIS_PORT))
            ...
            r_conn.set("phase18:predictions", orjson.dumps(fresh_signals).decode())
            logger.info("Canli firsatlar tablosu guncellendi", signal_count=len(fresh_signals))
        except Exception as sync_err:
            logger.warning("Sinyal senkronizasyonunda uyari", error=str(sync_err))

        logger.info("Master Startup Catch-up tamamlandi, tum sistem bugune esitlendi")
        return results

    def __repr__(self) -> str:
        """MasterStartupCatchup temsili."""
        return f"MasterStartupCatchup(calendar={self.calendar!r})"


master_catchup: MasterStartupCatchup = MasterStartupCatchup()
