...
logger.info("Hedef: OOS CAGR > %100, Sharpe > 1.0")
...
ALL_TICKERS = list(dict.fromkeys(bist_universe.BIST_ALL_TICKERS))
if not ALL_TICKERS:
    ALL_TICKERS = list(bist_universe._updater.get_universe().keys())
...
def evaluate(name, daily_ret_full, daily_ret_oos) -> Any:
    ...
    tag = ""
    if c_oos > 100 and s_oos > 1.0:
        tag = " *** HEDEF TUTTU ***"
    elif c_oos > 50 and s_oos > 0.5:
        tag = " ** UMUT VERICI"
    elif c_oos > 0:
        tag = " * POZITIF"
    logger.info(
        f"   {name:<40} Full:%{c_full:>6.1f} Sh:{s_full:>5.2f} | OOS:%{c_oos:>7.1f} Sh:{s_oos:>5.2f} DD:%{dd_oos:>6.1f}{tag}"
    )
    results.append(
        {
            "name": name,
...
def breakout_strategy(prices, returns, window=252, top_n=10) -> Any:
    ...
    for i in range(1, len(monthly_idx)):
        dt = monthly_idx[i - 1]
        dt_next = monthly_idx[i]
        if dt - pd.Timedelta(days=window) < prices.index[0]:
            continue
        p_now = prices.loc[:dt]
        if len(p_now) < window // 2:
            continue
        scores = {}
        for t in prices.columns:
            s = p_now[t].dropna()
...
def vol_breakout(prices, returns, vol_window=20, mom_window=5, top_n=10) -> Any:
    ...
    for i in range(1, len(monthly_idx)):
        dt = monthly_idx[i - 1]
        dt_next = monthly_idx[i]
        if dt - pd.Timedelta(days=vol_window * 3) < prices.index[0]:
            continue
        scores = {}
        for t in prices.columns:
            r = returns.loc[:dt][t].dropna()
            if len(r) < vol_window * 2:
                continue
...
def trend_filtered_momentum(prices, returns, top_n=10, lb=126) -> Any:
    ...
    for i in range(1, len(monthly_idx)):
        ...
        for t in prices.columns:
            ...
            last = p.iloc[-1]
            if last < ma200:
                continue  # Trend yukari degil, atla
...
for r in results[:20]:
    ...
    logger.info(
        f"{r['name']:<42} {r['oos_cagr']:>8.1f}% {r['oos_sh']:>7.2f} {r['oos_dd']:>7.1f}% {r['full_cagr']:>7.1f}% {tag}"
    )
