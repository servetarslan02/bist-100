...
def run_candidate(eval_dates, features_by_ticker, xu100_close, trainer, candidate_name, initial_capital=10_000_000.0):
    ...
    for step_i, current_date in enumerate(eval_dates):
        ...
        for i, tk in enumerate(day_tickers):
            ...
            alpha_ema = BASE_PARAMS["ema_alpha_fast"] if delta_s > BASE_PARAMS["ema_delta_thresh"] else BASE_PARAMS["ema_alpha_slow"]
        ...
        for tk, pos in list(positions.items()):
            ...
            should_exit = False
            if pnl_pct <= BASE_PARAMS["hard_stop_pct"]: should_exit = True
            elif pos["highest_price"] > pos["entry_price"] * 1.06 and cur_p < pos["highest_price"] * (1.0 - atr_buffer / 100.0): should_exit = True
            elif pnl_pct >= BASE_PARAMS["take_profit_pct"]: should_exit = True
            elif pos["days_held"] >= BASE_PARAMS["min_hold_days"] and smoothed_scores[tk] < BASE_PARAMS["signal_reversal_thresh"]: should_exit = True
            elif pos["days_held"] >= BASE_PARAMS["max_hold_days"]: should_exit = True
            ...
            if should_exit:
                ...
                trades += 1
                if net_val > pos["shares"] * pos["entry_price"]: wins += 1
        ...
        if slots > 0 and len(top_cand) > 0 and portfolio_cash > BASE_PARAMS["min_cash_to_open"]:
            ...
            for rank_idx, c in enumerate(top_cand[:slots]):
                alloc_pct = BASE_PARAMS["top1_alloc_pct"] if (rank_idx == 0 and c["score"] > BASE_PARAMS["conviction_score_min"]) else BASE_PARAMS["default_alloc_pct"]
