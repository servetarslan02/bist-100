...
def run_structural_discovery(eval_dates, features_by_ticker, stock_data, xu100_close, trainer):
    ...
    for step_i, current_date in enumerate(eval_dates):
        ...
        for i, tk in enumerate(day_tickers):
            ...
            cand.append({"ticker": tk, "score": score, "close": float(day_rows[i]["close"]), "fwd_5d": fwd_5d, "atr_pct": float(day_rows[i].get("atr_pct", 3.0))})
            ...
            for oc in oracle_top_4:
                if oc["fwd_5d"] > 0.05:
                    if oc["score"] < 0.10:
                        ...
                        if FROZEN_PARAMS["max_pos"].get(current_regime, 2) - len(positions) <= 0 or portfolio_cash < 200_000:
            ...
            for rank_idx, c in enumerate(top_cand_v3[:slots]):
                ...
                if shares > 0:
                    ...
                    positions[c["ticker"]] = {
                        ...
                        "highest_price": c["close"], "lowest_price": c["close"], "atr_pct": c["atr_pct"]
                    }
