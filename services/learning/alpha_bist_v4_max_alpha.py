...
V4_PARAMS = {
    ...
    "min_score": {"BULL_TREND": 0.08, "LOW_VOLATILITY": 0.10, "SIDEWAYS_RANGE": 0.20, "BEAR_MARKET": 0.28, "HIGH_VOLATILITY": 0.22},
...
def run_v4_strategy(eval_dates, features_by_ticker, xu100_close, trainer, initial_capital=10_000_000.0, label="V4_Max_Alpha"):
    portfolio_cash = initial_capital
    positions = {}
    equity_curve = []
    daily_rets = []
    
    regime_pnl = {r: {"pnl": 0.0, "trades": 0, "wins": 0} for r in ["BULL_TREND", "BEAR_MARKET", "SIDEWAYS_RANGE", "HIGH_VOLATILITY", "LOW_VOLATILITY", "V_DIP_RECOVERY"]}
    ...
    for step_i, current_date in enumerate(eval_dates):
        ...
        for i, tk in enumerate(day_tickers):
            ...
            alpha_ema = V4_PARAMS["ema_alpha_fast"] if delta_s > V4_PARAMS["ema_delta_thresh"] else V4_PARAMS["ema_alpha_slow"]
            smoothed_scores[tk] = alpha_ema * raw_c + (1.0 - alpha_ema) * smoothed_scores[tk]
            cand.append({"ticker": tk, "score": smoothed_scores[tk], "close": float(row["close"]), "atr_pct": float(row.get("atr_pct", 3.0))})

            for m in MODELS:
                pending_evals.append({"eval_date": current_date + timedelta(days=7), "model": m, "is_correct": ((1 if batch_sigs[tk][m] > 0 else -1) == (1 if ret_5d > 0 else -1))})
        ...
        for tk, pos in list(positions.items()):
            ...
            should_exit = False
            if pnl_pct <= V4_PARAMS["hard_stop_pct"]: should_exit = True
            elif pos["highest_price"] > pos["entry_price"] * 1.06 and cur_p < pos["highest_price"] * (1.0 - atr_buffer / 100.0): should_exit = True
            elif pnl_pct >= V4_PARAMS["take_profit_pct"]: should_exit = True
            elif pos["days_held"] >= V4_PARAMS["min_hold_days"] and smoothed_scores[tk] < V4_PARAMS["signal_reversal_thresh"]: should_exit = True
            elif pos["days_held"] >= V4_PARAMS["max_hold_days"]: should_exit = True
        ...
        if slots > 0 and len(top_cand) > 0 and portfolio_cash > V4_PARAMS["min_cash_to_open"]:
            ...
            for rank_idx, c in enumerate(top_cand[:slots]):
                ...
                alloc_pct = V4_PARAMS["top1_alloc_pct"] if (rank_idx == 0 and c["score"] > V4_PARAMS["conviction_score_min"]) else V4_PARAMS["default_alloc_pct"]
    ...
    max_dd_xu = abs(((pd.Series(equity_xu100) - pd.Series(equity_xu100).cummax()) / pd.Series(equity_xu100).cummax()).min()) * 100.0
    ...
    return {
        "label": label, "final_equity": eq_s.iloc[-1],
        "tot_ret": tot_ret, "tot_ret_xu": tot_ret_xu, "cagr": cagr,
        "max_dd": max_dd, "max_dd_xu": max_dd_xu,
        "win_rate": win_rate, "profit_factor": profit_factor,
        "sortino": sortino, "calmar": calmar,
        "turnover": turnover, "costs": total_costs,
        "upside_cap": upside_cap, "trades": trades,
