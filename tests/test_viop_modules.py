...
class TestArbitrage:
    ...
    def test_overpriced_futures(self):
        ...
        arb = FuturesSpotArbitrage()
        result = arb.analyze(
            spot_price=10000, futures_price=10500, risk_free_rate=0.15, dividend_yield=0.02, time_to_expiry=0.25
        )
    ...
    def test_underpriced_futures(self):
        ...
        arb = FuturesSpotArbitrage()
        result = arb.analyze(
            spot_price=10000, futures_price=9600, risk_free_rate=0.15, dividend_yield=0.02, time_to_expiry=0.25
        )
        assert result.arbitrage_opportunity
...
class TestBacktest:
    ...
    def test_iron_condor_backtest(self):
        ...
        prices = [
            {"date": date(2026, 1, 1) + timedelta(days=i), "close": 100 + np.random.uniform(-1, 1)} for i in range(60)
        ]
