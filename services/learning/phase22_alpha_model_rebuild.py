...
def run_phase_22() -> Any:
    ...
    logger.info("1-5. ZORUNLU AUDIT (Leakage & Integrity):")
    ...
    df_all["label"] = df_all.groupby("date")["target_5d_ret"].transform(
    ...
    for d, grp in res_df.groupby("date"):
        ...
        q_mod = (
            pd.qcut(grp["oos_score"], 5, labels=False, duplicates="drop")
            if len(np.unique(grp["oos_score"])) > 4
            else None
        )
        ...
        if q_mod is not None:
            for q in range(5):
                m_rec[f"Q{q + 1}"] = grp.loc[q_mod == q, "ex_5d"].mean()
    ...
    logger.info("Null Shuffled Spread: %0.000 (By definition, random Q5-Q1 = 0)")
    logger.info(
        f"95% CI              : [%{np.percentile(boot_means, 2.5):.3f}, %{np.percentile(boot_means, 97.5):.3f}]"
    )
    ...
    if res_metrics["ic_mod"].mean() > res_metrics["ic_vol"].mean() and np.percentile(boot_means, 2.5) > 0:
        ...
        logger.info(
            "Yeni model, saf Volatility benchmark'ından daha fazla bilgi taşıyor ve %95 CI pozitif. Production entegrasyonu için ONAYLANDI."
        )
    else:
        logger.info("FINAL DECISION: REJECT")
        logger.info(
            "Yeni LambdaRank modeli saf Volatilite'yi yenemedi veya stabil değil. Fazladan ML katmanı değer yaratmıyor. Sadece 'volatility_20d' thresholding kullanılmalıdır."
        )
