...
def test_walk_forward_split():
    ...
    passed += 1
    print(f"  ✓ Purge gap verified (min {engine.purge_days + 1} days)")
    ...
    passed += 1
    print(f"  ✓ Embargo gap verified (min {engine.embargo_days + 1} days)")
    ...
    passed += 1
    print(f"  ✓ Train={engine.train_days}d, Test={engine.test_days}d verified")
...
def test_walk_forward_run():
    ...
    passed += 1
    print(f"  ✓ Metrics: return={result.avg_test_return:.4f}, sharpe={result.avg_test_sharpe:.4f}")

    # 3. Precision@K
    assert 0 <= result.avg_precision_at_5 <= 1
    assert 0 <= result.avg_precision_at_10 <= 1
    passed += 1
    print(f"  ✓ Precision@K: P@5={result.avg_precision_at_5:.3f}, P@10={result.avg_precision_at_10:.3f}")

    # 4. IC
    assert -1 <= result.avg_ic <= 1
    passed += 1
    print(f"  ✓ IC: {result.avg_ic:.4f}")
...
def main():
    print("=" * 60)
    print("  FAZ 4 — Backtest & Evaluation Test Suite")
    print("=" * 60)
