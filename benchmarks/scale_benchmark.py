...
def main():
    ...
    report = {"date": datetime.now().isoformat(), "days": DAYS,
              "lookback": LOOKBACK, "scales": {}}
    ...
    for scale in SCALES:
        ...
        print(f"  PANEL : {entry['panel']['wall_seconds']:>8.2f}s | "
        ...
        if scale <= LEGACY_MAX_SCALE:
            ...
            print(f"  LEGACY: {entry['legacy']['wall_seconds']:>8.2f}s | "
            ...
            same = (entry["legacy"]["total_scans"] == entry["panel"]["total_scans"]
            ...
            entry["legacy_extrapolated"] = {
                ...
                "note": (f"100 hisse ölçümünden ekstrapolasyon "
        ...
        if "legacy" in entry:
            ...
            entry["speedup"] = round(entry["legacy_extrapolated"]["wall_seconds"] / max(entry["panel"]["wall_seconds"], 1e-9), 1)
    ...
    for scale, e in report["scales"].items():
        p = e["panel"]
        lines.append(f"| {scale}h/1y | **panel (yeni)** | {p['wall_seconds']}s | {p['scans_per_second']:,} | "
                     f"{p['feature_seconds']}s | {p['peak_rss_mb']}MB | %{p['cpu_percent']} |")
        if "legacy" in e:
            l = e["legacy"]
            lines.append(f"| {scale}h/1y | legacy (eski) | {l['wall_seconds']}s | {l['scans_per_second']:,} | "
                         f"{l['feature_seconds']}s | {l['peak_rss_mb']}MB | %{l['cpu_percent']} |")
        else:
            l = e["legacy_extrapolated"]
            lines.append(f"| {scale}h/1y | legacy (eski, ekstra.) | ~{l['wall_seconds']:,.0f}s | {l['scans_per_second']:,} | - | - | - |")
