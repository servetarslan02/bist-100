...
def test_normal_buy_flow():
    ...
    print(f'  [PASS] Transaction cost calculated: total_cost={cost["total_cost"]} TL ({cost["total_cost_pct"]}%)')

def test_risk_vetoed_buy():
...
def test_model_failure_fallback():
    print('=== 3. MODEL FAILURE FALLBACK TEST ===')
    ...
    print(f'  [PASS] Canonical scoring model failure fallback verified: rule_score={score_res.opportunity_score}')
...
def test_data_missing_fallback():
    ...
    print('  [PASS] Data quality missing/corrupted data mask applied')

def test_scheduler_job_execution():
    print('=== 5. SCHEDULER JOB EXECUTION TEST ===')
    ...
    print(f'  [PASS] UnifiedScheduler initialized with {st["job_configs"]} registered job configurations')

def test_paper_order_execution():
    print('=== 6. PAPER ORDER GENERATION & VIRTUAL BALANCE TEST ===')
    ...
    print(f'  [PASS] Paper order created: {order.ticker} {order.side} {order.quantity} @ {order.price}')
...
if __name__ == '__main__':
    ...
    print('=== ALL 6 PHASE 5 END-TO-END SCENARIOS PASSED ===')
