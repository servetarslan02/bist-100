    ...
    async def fetch_stock_price(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if data:
                return {
                    "ticker": ticker,
                    "price": data.get("lastPrice") or data.get("lastPrice", 0),
                    "change_pct": data.get("changePercent") or data.get("changePercent", 0),
                    "volume": data.get("volume") or data.get("volume", 0),
                    "high": data.get("high") or data.get("high", 0),
                    "low": data.get("low") or data.get("low", 0),
                    "open": data.get("open") or data.get("open", 0),
                    "close": data.get("close") or data.get("close", 0),
                    "bid": data.get("bidPrice") or data.get("bidPrice", 0),
                    "ask": data.get("askPrice") or data.get("askPrice", 0),
                    "best_bid_volume": data.get("bidVolume") or data.get("bidVolume", 0),
                    "best_ask_volume": data.get("askVolume") or data.get("askVolume", 0),
