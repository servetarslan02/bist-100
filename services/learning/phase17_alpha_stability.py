...
def run_alpha_stability_audit() -> Any:
    ...
    for i, b in enumerate(blocks):
        ...
        if spr > 0:
            pos_blocks += 1
        logger.info(
            f"Block {i + 1} | N={len(b):<2} | Top3: %{t3:>5.2f} | Top5: %{t5:>5.2f} | Bot5: %{b5:>5.2f} | Spread (Top5-Rand): %{spr:>5.2f}"
        )
    ...
    for reg in ["BULL_TREND", "BEAR_MARKET", "SIDEWAYS_RANGE"]:
        ...
        if len(d_sub) > 0:
            ...
            logger.info(
                f"{reg:15} | N={len(d_sub):<3} | Ranker: %{act:>5.2f} | Random: %{rnd:>5.2f} | Spread: %{spr:>5.2f} | Pos Days: %{pos_days:.1f}"
            )
    ...
    logger.info(
        f"En iyi %20 Gün Çıkarıldığında: %{mean_no_20:.3f} (DİKKAT: Eğer bu eksiye düşüyorsa Alpha sadece birkaç güne bağlıdır!)"
    )
    ...
    horizons = [
        (1, "top5_1d", "shuf5_1d"),
        (5, "top5_5d", "shuf5_5d"),
        (10, "top5_10d", "shuf5_10d"),
        (20, "top5_20d", "shuf5_20d"),
    ]
    for h, top_col, shuf_col in horizons:
        # Check if the target exists
        if df_m[top_col].isnull().all():
            continue
        mean_top = df_m[top_col].mean()
        mean_shuf = df_m[shuf_col].mean()
        spread = mean_top - mean_shuf
        logger.info(
            f"{h:>2}D Horizon | Top-5: %{mean_top:>5.2f} | Shuffled-5: %{mean_shuf:>5.2f} | Spread: %{spread:>5.2f}"
        )
    ...
    if p_value > 0.05:
        logger.info(
            "-> DİKKAT: Top-5 Alpha 1000 rastgele teste karşı istatistiksel anlamlılığını (p < 0.05) KORUYAMADI."
        )
