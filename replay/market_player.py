...
class MarketPlayer:
    """Geçmiş piyasa verisini oynatan motor — multi-index destekli."""
    ...
    def load_data(
        ...
        """
        result = {"start_date": start_date, "end_date": end_date, "source": source, "universe": universe}
            ...
            if source == "yfinance":
                ...
                if not raw.empty:
                    ...
                    if frames:
                        ...
                        logger.info("Market data loaded", tickers=len(frames), ticks=self._total_ticks)
                    else:
                        result["status"] = "empty"
                else:
                    result["status"] = "empty"
    ...
    def play(self, start_index: int = 0, max_ticks: Optional[int] = None) -> Generator[TickData, None, None]:
        ...
        logger.info("Playback started", start=start_index, end=end_index, speed=self._speed.value)
        ...
        try:
            ...
            logger.info("Playback finished", ticks=self._ticks_processed)
