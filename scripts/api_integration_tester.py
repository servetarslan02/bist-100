...
def main():
    print("=" * 80)
    print("  ALPHA BIST — CANLI API & ENTEGRASYON TESTİ (Response Times)")
    print("=" * 80)
    
    schema = fetch_openapi_schema()
    if not schema:
        print("API şeması alınamadı. Servis ayakta mı?")
        # Fallback to checking just the health endpoint
        endpoints = {"/health": {"get": {}}}
    else:
        endpoints = schema.get("paths", {})
        print(f"Toplam {len(endpoints)} API route'u bulundu.")
        print("-" * 80)
        
    print(f"{'Metot':<7} | {'Endpoint':<40} | {'Durum':<10} | {'Yanıt Süresi'}")
    print("-" * 80)
    ...
    for path, methods in endpoints.items():
        for method in methods.keys():
            ...
            print(f"{method.upper():<7} | {path:<40} | {status_str:<10} | {time_str}")
            
            if res["error"] and res["status"] != "SKIPPED":
                print(f"          └─ Hata: {res['error']}")

    print("-" * 80)
    print(f"Toplam: {success+skipped+failed} | Başarılı/Beklenen: {success} | Atlanan: {skipped} | Hatalı: {failed}")
    print("=" * 80)
