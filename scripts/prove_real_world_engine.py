...
def run_proof():
    print("=" * 72)
    print("  ALPHA BIST 30Y ML + RISK PARITY MOTORU GERCEK DUNYA KANITI")
    print("=" * 72)
    ...
    for mf in model_files:
        if os.path.exists(mf):
            ...
            print(f"  [OK] Model: {os.path.basename(mf):<25} | Boyut: {size_kb:.1f} KB | Egitim: {mtime}")
        else:
            print(f"  [--] Model eksik: {mf}")
    ...
    try:
        ...
        with urllib.request.urlopen(req) as resp:
            ...
            print(f"  [OK] Dinamik Makro Yorumu   : {macro.get('macro_commentary')[:90]}...")
    except Exception as e:
        print(f"  [--] Makro API hatasi: {e}")
