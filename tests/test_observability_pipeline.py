...
from services.core.observability import PrometheusMetrics, DistributedTracing, prometheus_metrics
...
class TestPrometheusMetricsEngine:
    ...
    def test_counters_and_gauges(self):
        ...
        assert 'orders_total{side="BUY",ticker="THYAO"} 3.0' in text or 'orders_total{ticker="THYAO",side="BUY"} 3.0' in text
        assert 'orders_total{side="SELL",ticker="ASELS"} 1.0' in text or 'orders_total{ticker="ASELS",side="SELL"} 1.0' in text
    ...
    def test_prometheus_text_exposition_format(self):
        metrics = PrometheusMetrics()
...
class TestOpenTelemetryModule:
    ...
    def test_otel_setup_and_tracer(self):
...
class TestMetricsEndpointIntegration:
    ...
    def test_metrics_endpoint_returns_prometheus_text(self, client):
        ...
        assert "prometheus" in resp.headers.get("content-type", "").lower() or "text/plain" in resp.headers.get("content-type", "").lower()

    def test_health_endpoint_checks_services(self, client):
