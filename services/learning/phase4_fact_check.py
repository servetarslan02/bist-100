...
def run_fact_check_audit(eval_dates, features_by_ticker, xu100_close, trainer):
    ...
    for step_i, current_date in enumerate(eval_dates):
        ...
        daily_breadth_logs.append({
            ...
            "ret_3d": ret_3d, "ret_5d": ret_5d, "ret_10d": ret_10d, "ret_20d": ret_20d,
            "vol_20d": vol_20d
        })
        ...
        for i, tk in enumerate(day_tickers):
            ...
            cand.append({"ticker": tk, "score": smoothed_scores[tk], "close": float(day_rows[i]["close"]), "atr_pct": float(day_rows[i].get("atr_pct", 3.0))})
        ...
        for tk, pos in list(positions.items()):
            ...
            exit_reason = None
            if pnl_pct <= FROZEN_PARAMS["hard_stop_pct"]: exit_reason = "HARD_STOP"
            elif pos["highest_price"] > pos["entry_price"] * 1.06 and cur_p < pos["highest_price"] * (1.0 - atr_buffer / 100.0): exit_reason = "TRAILING_STOP"
            elif pnl_pct >= FROZEN_PARAMS["take_profit_pct"]: exit_reason = "TAKE_PROFIT"
            elif pos["days_held"] >= FROZEN_PARAMS["min_hold_days"] and smoothed_scores[tk] < FROZEN_PARAMS["signal_reversal_thresh"]: exit_reason = "SIGNAL_REVERSAL"
            elif pos["days_held"] >= FROZEN_PARAMS["max_hold_days"]: exit_reason = "MAX_HOLD"
            ...
            for rank_idx, c in enumerate(top_cand[:slots]):
                ...
                if shares > 0:
                    ...
                    positions[c["ticker"]] = {
                        ...
                        "atr_pct": c["atr_pct"], "regime": regime_tag
                    }
