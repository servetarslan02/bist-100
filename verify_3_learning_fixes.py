...
for opp in opps[:3]:
    score_val = opp.get('score', 0.0)
    conf_val = opp.get('confidence', 0.0)
    target_val = opp.get('target_return_pct', 0.0)
    print(f"    - {opp['symbol']}: Güven: {conf_val:.2f}, Skor: {score_val:.2f}, Hedef: +%{target_val}")
