...
class IngestionMetrics:
    ...
    # Provider metrics
    def record_provider_request(self, provider: str, data_type: str, status: str, latency_s: float):
    ...
    @contextmanager
    def track_provider(self, provider: str, data_type: str):
    ...
    # Rate limiter metrics
    def record_rate_limit_wait(self, provider: str, wait_seconds: float):
        """Rate limiter bekleme kaydı."""
        if not self._enabled:
            return
        RL_WAIT_SECONDS.labels(provider=provider).observe(wait_seconds)

    def record_rate_limit_rejected(self, provider: str):
    ...
    # Data quality metrics
    def record_quality_score(self, ticker: str, source: str, score: float):
        """Kalite skoru kaydı."""
        if not self._enabled:
            return
        DQ_SCORE.labels(ticker=ticker, source=source).observe(score)

    def record_reconciliation_conflict(self, ticker: str):
    ...
    # Dedup metrics
    def record_dedup_duplicate(self, event_type: str):
        """Tekrar event kaydı."""
        if not self._enabled:
            return
    ...
    # Pipeline metrics
    @contextmanager
    def track_pipeline(self, pipeline_type: str):
    ...
    def record_pipeline_event(self, event_type: str, source: str):
    ...
    # PIT metrics
    def record_pit_violation(self, data_type: str):
    ...
    # Incremental metrics
    def record_incremental_fetch(self, ticker: str):
        """Incremental fetch kaydı."""
        if not self._enabled:
            return
        INC_FETCHES.labels(ticker=ticker).inc()

    def record_incremental_skip(self, ticker: str):
