...
def run_audit_simulation(eval_dates, features_by_ticker, xu100_close, trainer, initial_capital=10_000_000.0):
    ...
    for step_i, current_date in enumerate(eval_dates):
        ...
        for i, tk in enumerate(day_tickers):
            ...
            cand.append({
                "ticker": tk, "score": smoothed_scores[tk], "raw_score": raw_c,
        ...
        for tk, pos in list(positions.items()):
            ...
            elif pos["highest_price"] > pos["entry_price"] * 1.06 and cur_p < pos["highest_price"] * (1.0 - atr_buffer / 100.0):
                exit_reason = "TRAILING_STOP"
            elif pnl_pct >= FROZEN_PARAMS["take_profit_pct"]:
                exit_reason = "TAKE_PROFIT"
            elif pos["days_held"] >= FROZEN_PARAMS["min_hold_days"] and smoothed_scores[tk] < FROZEN_PARAMS["signal_reversal_thresh"]:
        ...
        invested_pre_open = sum(p["shares"] * float(features_by_ticker[t].loc[current_date]["close"]) for t, p in positions.items())
        cash_ratio_pre = portfolio_cash / (portfolio_cash + invested_pre_open) if (portfolio_cash + invested_pre_open) > 0 else 1.0
        ...
        if slots > 0 and len(top_cand) > 0 and portfolio_cash > FROZEN_PARAMS["min_cash_to_open"]:
            ...
            for rank_idx, c in enumerate(top_cand[:slots]):
                alloc_pct = FROZEN_PARAMS["top1_alloc_pct"] if (rank_idx == 0 and c["score"] > FROZEN_PARAMS["conviction_score_min"]) else FROZEN_PARAMS["default_alloc_pct"]
                ...
                if shares > 0:
                    ...
                    positions[c["ticker"]] = {
                        "shares": shares, "entry_price": c["close"], "entry_date": current_date,
                        "days_held": 0, "highest_price": c["close"], "lowest_price": c["close"],
                        "atr_pct": c["atr_pct"], "regime": current_regime, "regime_tag": regime_tag
                    }
