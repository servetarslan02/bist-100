...
POST_PAYLOADS = {
    ...
    "/api/v1/risk/check": {"portfolio": {"THYAO": 1000}, "proposed_trade": {"ticker": "GARAN", "action": "BUY", "quantity": 100, "price": 100.0}},
    ...
    "/api/v1/backtests/run": {"strategy": "momentum", "tickers": ["THYAO"], "start_date": "2026-01-01", "end_date": "2026-06-01"},
    "/api/v1/backtests/walk-forward": {"strategy": "momentum", "tickers": ["THYAO"]},
    "/api/v1/learning/cycle": {"force": False},
    "/api/v1/learning/record_prediction": {"model_name": "momentum", "ticker": "THYAO", "prediction": 0.05, "confidence": 0.8},
    ...
    # VIOP
    "/api/v1/viop/options/price": {"spot_price": 300.0, "strike_price": 300.0, "time_to_maturity": 0.25, "risk_free_rate": 0.45, "volatility": 0.30, "option_type": "call"},
    "/api/v1/viop/options/implied-vol": {"target_price": 15.0, "spot_price": 300.0, "strike_price": 300.0, "time_to_maturity": 0.25, "risk_free_rate": 0.45, "option_type": "call"},
    "/api/v1/viop/greeks": {"spot_price": 300.0, "strike_price": 300.0, "time_to_maturity": 0.25, "risk_free_rate": 0.45, "volatility": 0.30, "option_type": "call"},
    "/api/v1/viop/strategies/analyze": {"strategy": "straddle", "spot_price": 300.0, "strike_price": 300.0, "time_to_maturity": 0.25, "risk_free_rate": 0.45, "volatility": 0.30},
    ...
    "/api/v1/viop/arbitrage": {"spot_price": 300.0, "future_price": 315.0, "time_to_maturity": 0.25, "risk_free_rate": 0.45, "dividend_yield": 0.0},
    "/api/v1/viop/parity": {"spot_price": 300.0, "strike_price": 300.0, "call_price": 20.0, "put_price": 15.0, "time_to_maturity": 0.25, "risk_free_rate": 0.45},
...
def test_endpoint(method: str, path: str, url: str) -> dict:
    ...
    headers = {
        "User-Agent": "Alpha-APITester/2.0",
...
def main():
    print("=" * 90)
    print("  ALPHA BIST — DERİNLEMESİNE (100% KAPSAM) CANLI API & ENTEGRASYON DENETİMİ")
    print("=" * 90)
    
    schema = fetch_openapi_schema()
    if not schema:
        print("API şeması alınamadı.")
        return
    ...
    print(f"Toplam Denetlenecek Uç Nokta Sayısı: {len(all_endpoints)}")
    print("-" * 90)
    print(f"{'Metot':<7} | {'Endpoint':<45} | {'Durum':<12} | {'Yanıt Süresi'}")
    print("-" * 90)
    ...
    for method, path in all_endpoints:
        ...
        print(f"{method:<7} | {path:<45} | {status_str:<12} | {time_str}")
        if res["error"] and (not isinstance(status_val, int) or status_val >= 500):
            print(f"          └─ [KRİTİK HATA]: {res['error']}")
        elif res["error"] and status_val in (400, 404, 422):
            print(f"          └─ (Bilgi/Validasyon): {res['error']}")
            
    print("-" * 90)
    print(f"SONUÇ: Toplam: {len(all_endpoints)} | Başarılı (2xx/3xx): {success} | İstemci/Validasyon (4xx): {client_err} | Sunucu Hatası (5xx): {server_err}")
    print("=" * 90)
    ...
    for res in results:
        ...
        if not isinstance(status_val, int) or status_val >= 500:
            time_str = f"{res['time_ms']:.1f} ms" if res["time_ms"] is not None else "-"
            print(f"{res['method']:<7} | {res['path']:<45} | {res['status']:<12} | {time_str}")
    ...
    for res in results:
        ...
        if isinstance(status_val, int) and status_val < 500 and res["time_ms"] is not None and res["time_ms"] > 2000:
            print(f"{res['method']:<7} | {res['path']:<45} | {res['status']:<12} | {res['time_ms']:.1f} ms")
