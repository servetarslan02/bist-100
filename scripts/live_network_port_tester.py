...
PORTS_TO_CHECK = [
    ...
    {"name": "Alpha API (FastAPI)", "host": "127.0.0.1", "port": 8000, "protocol": "HTTP", "url": "http://127.0.0.1:8000/health"},
    ...
    {"name": "ClickHouse 1 (HTTP)", "host": "127.0.0.1", "port": 8123, "protocol": "HTTP", "url": "http://127.0.0.1:8123/ping"},
    {"name": "ClickHouse 1 (Native)", "host": "127.0.0.1", "port": 9002, "protocol": "TCP"},
    {"name": "ClickHouse 2 (HTTP)", "host": "127.0.0.1", "port": 8124, "protocol": "HTTP", "url": "http://127.0.0.1:8124/ping"},
    ...
    {"name": "NATS Monitor", "host": "127.0.0.1", "port": 8222, "protocol": "HTTP", "url": "http://127.0.0.1:8222/healthz"},
    {"name": "QuestDB (Web/REST)", "host": "127.0.0.1", "port": 9000, "protocol": "HTTP", "url": "http://127.0.0.1:9000/"},
    ...
    {"name": "Prometheus", "host": "127.0.0.1", "port": 9090, "protocol": "HTTP", "url": "http://127.0.0.1:9090/-/healthy"},
    {"name": "Grafana Dashboard", "host": "127.0.0.1", "port": 3001, "protocol": "HTTP", "url": "http://127.0.0.1:3001/api/health"},
    {"name": "Redis Exporter", "host": "127.0.0.1", "port": 9121, "protocol": "HTTP", "url": "http://127.0.0.1:9121/health"},
    {"name": "Postgres Exporter", "host": "127.0.0.1", "port": 9187, "protocol": "TCP"},
    {"name": "Frontend Dashboard", "host": "127.0.0.1", "port": 3000, "protocol": "HTTP", "url": "http://127.0.0.1:3000/"},
]
...
def run_suite():
    print("=" * 80)
    print("  ALPHA BIST — CANLI AĞ, PORT & SERVİS ENTEGRASYON TESTİ")
    print(f"  Zaman: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    print(f"{'#':<3} | {'Servis Adı':<26} | {'Port':<6} | {'Protokol':<8} | {'Gecikme':<9} | {'Durum'}")
    print("-" * 80)
    ...
    for i, target in enumerate(PORTS_TO_CHECK, 1):
        ...
        print(f"{i:<3} | {name:<26} | {port:<6} | {proto:<8} | {ms:>6.1f} ms | {status_symbol} ({msg})")

    print("-" * 80)
    print(f"  Toplam Port: {len(PORTS_TO_CHECK)} | Başarılı: {success_count} | Başarısız: {fail_count}")
    print("=" * 80)
