...
def test_kap_extractor() -> Any:
    ...
    passed += 1
    logger.info(f"  ✓ Temettü: type={result.event_type}, impact={result.financial_impact:.2f}")
    ...
    assert result.financial_impact > 0
    passed += 1
    logger.info(f"  ✓ Yatırım: type={result.event_type}, impact={result.financial_impact:.2f}")

    # 3. Sözleşme
    result = kap_extractor.extract("TUPRS", "KAP-003", "Yeni petrol sözleşmesi imzalandı")
    assert result.event_type == "CONTRACT"
    passed += 1
    logger.info(f"  ✓ Sözleşme: type={result.event_type}, impact={result.financial_impact:.2f}")
    ...
    assert result.financial_impact < 0
    passed += 1
    logger.info(f"  ✓ Dava: type={result.event_type}, impact={result.financial_impact:.2f}")

    # 5. Beklenmediklik
    result = kap_extractor.extract("THYAO", "KAP-005", "Sürpriz kar payı dağıtımı açıklandı")
    assert result.surprise_score > 0.5
    passed += 1
    logger.info(f"  ✓ Beklenmediklik: surprise={result.surprise_score:.2f}")
    ...
    passed += 1
    logger.info(f"  ✓ Belirsizlik: uncertainty={result.uncertainty:.2f}")
    ...
    passed += 1
    logger.info(f"  ✓ Sektör: {result.affected_sectors}")

    # 8. Bilinmeyen olay
    result = kap_extractor.extract("TEST", "KAP-008", "Genel kurul toplantısı yapılacak")
    assert result.event_type == "UNKNOWN"
    passed += 1
    logger.info(f"  ✓ Bilinmeyen: type={result.event_type}")

    return passed, failed


def test_sector_chain() -> Any:
    ...
    passed += 1
    logger.info(f"  ✓ Enerji→Havacılık: {aviation_impact['impact']:.2f} ({aviation_impact['reason']})")
    ...
    passed += 1
    logger.info(f"  ✓ Banka→İnşaat: {constr_impact['impact']:.2f} ({constr_impact['reason']})")

    # 3. Bilinmeyen sektör
    impacts = sector_chain.compute_chain_impact("UNKNOWN", 1.0)
    assert len(impacts) == 0
    passed += 1
    logger.info(f"  ✓ Bilinmeyen sektör: {len(impacts)} etki")
    ...
    passed += 1
    logger.info(f"  ✓ Negatif yön: Enerji→Havacılık={aviation_impact['impact']:.2f}")
...
def main() -> Any:
    """Otomatik eklendi."""
    logger.info("=" * 60)
    logger.info("  FAZ 6 — KAP Extractor & Sector Chain Test Suite")
    logger.info("=" * 60)
