...
mock_prices = {
    "THYAO": [
        ...
        {"date": "2026-01-03", "open": 106.0, "high": 107.0, "low": 90.0, "close": 98.0, "volume": 1500000},  # Intraday drop to 90 (< stop 95)
...
if res.trades:
    ...
    print(f"  -> Sinyal Tarihi: 2026-01-01 | İcra Tarihi: {first_trade.entry_date} | İcra Fiyatı: {first_trade.entry_price:.2f} ₺")
    print(f"  -> Çıkış Nedeni (Stop-Loss): {first_trade.side} | Çıkış Tarihi: {first_trade.exit_date} | Çıkış Fiyatı: {first_trade.exit_price:.2f} ₺")
...
bear_input = DecisionInput(ticker="GARAN", price=120.0, regime="BEAR_MARKET", ml_score=65.0, ml_confidence=0.66, atr=3.5)
bear_dec = dec_engine.decide(bear_input)
print(f"  -> Ayı Piyasası (Skor: {bear_dec.score:.1f}, Güven: {bear_input.ml_confidence}) -> Karar: {bear_dec.action} | Gerekçe: {bear_dec.reasons[0]}")
...
bull_input = DecisionInput(
    ticker="GARAN", price=120.0, regime="BULL_TREND", ml_score=75.0, ml_confidence=0.70, atr=3.5,
    features={"rsi_14": 58.0, "trend_strength": 75.0, "macd_hist": 1.2, "supertrend_dir": 1},
    agent_score=70.0, agent_confidence=0.8
)
bull_dec = dec_engine.decide(bull_input)
print(f"  -> Boğa Piyasası (Skor: {bull_dec.score:.1f}, Güven: {bull_input.ml_confidence}) -> Karar: {bull_dec.action} | Hedef: {bull_dec.target_price:.2f} ₺ | Stop: {bull_dec.stop_price:.2f} ₺")
